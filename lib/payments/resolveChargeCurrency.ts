...
// that field is a rejected proposal, not an unbuilt one. Both currencies that
// reach Stripe travel in a payload next to `connectedAccountId`: a Connect
// destination charge on the PROFESSIONAL's own account, which has to support
// the currency. A brand-sheet field would be a control that looks
// authoritative, sits in a config file a tenant edits, and breaks payments the
// first time anyone sets it to something their pros' Stripe accounts cannot
// take.
//
 ...
 * Pass nothing at a site that has no booking column to read — the deposit and
 * no-show charges construct their own PaymentIntent rather than inheriting one.
 */
