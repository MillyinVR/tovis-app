...
const EVENT_LABELS: Record<NotificationEventKey, string> = {
  ...
  [NotificationEventKey.WAITLIST_OFFER_EXPIRED]: 'Offered time went unanswered',
  [NotificationEventKey.WAITLIST_CLIENT_LEFT]: 'Someone left your waitlist',
  ...
  // K15: deliberately NOT in CLIENT_NOTIFICATION_EVENT_KEYS or any category, on
  ...
  [NotificationEventKey.LOOK_COMMENT_REPLIED]: 'Replies to your comments',
  [NotificationEventKey.LOOK_LIKED]: 'Likes on your looks',
  [NotificationEventKey.LOOK_SAVED]: 'Saves on your looks',
  [NotificationEventKey.LOOK_NEW_FROM_FOLLOWED_PRO]: 'New looks from pros you follow',
  [NotificationEventKey.LOOK_MILESTONE_REACHED]: 'Look milestones',
...
const CATEGORY_DEFS: readonly CategoryDef[] = [
  ...
  },
  {
    ...
    eventKeys: [
      ...
      // The other two ends of the same waitlist-offer loop, both pro-facing: an
      // offer that lapsed unanswered, and a client who left while one was live.
      // They belong beside WAITLIST_JOINED so a pro who mutes "last-minute" mutes
      // the whole waitlist conversation, not two thirds of it.
