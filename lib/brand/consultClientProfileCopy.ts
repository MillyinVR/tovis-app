// Client descriptions only. Professional observations retain their original codes.
export const consultClientProfileValues: Readonly<Record<string, Readonly<Record<string, string>>>> = {
  currentTone: { ASHY: "Gray or silvery", NEUTRAL: "Neither golden nor silvery", GOLDEN: "Golden", COPPER: "Orange or reddish", RED: "Red", MIXED: "A mix of shades" },
  visibleCondition: { NO_VISIBLE_CONCERN: "No visible concern in these photos", POSSIBLE_COMPROMISE: "Your professional should check the condition in person" },
  density: { LOW: "Less full", MEDIUM: "Moderately full", HIGH: "Very full" },
  texture: { STRAIGHT: "Straight", WAVY: "Wavy", CURLY: "Curly", COILY: "Very tight curls", MIXED: "A mix of straight and curly areas" },
  "skinUndertone": {
...
export function clientConsultProfileValue(field: string, value: string): string {
  return consultClientProfileValues[field]?.[value] ?? 'Couldn’t tell from the photos'
}
