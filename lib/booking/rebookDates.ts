...
export function addDaysToYmd(ymd: string, days: number): string | null {
  ...
  return ymdToString(addDaysToYMD(p.year, p.month, p.day, days))
}
...
/** Thin wrapper over the shared `ymdInTimeZone` (which sanitizes the zone). */
export function todayYmdInTimeZone(timeZone: string): string {
  return ymdInTimeZone(new Date(), timeZone)
}

/**
 * Thin wrapper over the shared `ymdInTimeZoneFromIso`, keeping this module's
 * ''-on-missing contract (its form-state callers bind the result straight to
 * `<input value>`, where null is not a value).
 */
export function isoToYmdInTimeZone(
  ...
  if (!iso) return ''
  return ymdInTimeZoneFromIso(iso, timeZone) ?? ''
}
