...
| `docs/security/contact-lookup-hash-threat-model.md` | Documents SHA-256 vs HMAC-SHA256 contact lookup hash decision and migration plan. | Yes | IN PROGRESS | Tori / Security reviewer needed | File exists. Decision documented and HMAC-SHA256 v2 migration SHIPPED (legacy SHA-256 columns dropped in `20260601000000_drop_legacy_contact_lookup_hashes`). Remaining: security-reviewer sign-off only. |
...
| Analytics PII spot check completed | Tori | DONE | Yes | Yes | No | No | Partial | Spot check 2026-06-12 at `e6a52a8`. No third-party analytics SDK in `package.json`. Emission surfaces reviewed: `lib/observability/authEvents.ts` (sanitizer, emailHash-only, tested), `lib/observability/bookingEvents.ts` (Sentry tags/context carry route + entity IDs only), `lib/observability/looksSocialJobEvents.ts` (`sanitizeMeta` applied), `AttributionEvent.metaJson` (entity IDs, role, and relative-path-only `nextUrl` via `safeNextUrl`), and global Sentry scrub `scrubSentryEvent` → `redactAuditPayload` (`lib/observability/sentryConfig.ts:66`). No raw email/phone/name/address found in any emitted payload. Observability + logging suites passed locally 2026-06-12 (64 tests / 5 files incl. `authEvents.test.ts`, `looksSocialJobEvents.test.ts`). Remaining caveat: re-run the spot check if a third-party analytics SDK is ever added. |
...
| SHA-256 vs HMAC contact hash decision documented | Tori / Security reviewer needed | IN PROGRESS | Yes | N/A | N/A | N/A | No | Decision recorded in `docs/security/contact-lookup-hash-threat-model.md`. The HMAC-SHA256 migration has since shipped: versioned-keyring HMAC v2 hashes are the only read path (`lib/security/crypto/hashLookup.ts`), and legacy SHA-256 columns were dropped in `prisma/migrations/20260601000000_drop_legacy_contact_lookup_hashes`. Remaining: security-reviewer sign-off of the threat model. Recorded in `docs/launch-readiness/test-proof.md`. |
