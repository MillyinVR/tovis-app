...
| Media/private media | Media metadata/upload path works and private media cannot be publicly accessed in deployed storage policy. Run `pnpm proof:storage-policy` (scripts/proof-storage-policy.mjs) — expect 6/6 (anon-deny, signed-read, PUT upload, POST reproduces RLS, public read). | PASS DEPLOYED 2026-06-13 (6/6); see storage-policy-proof.md | Yes, if media enabled |
