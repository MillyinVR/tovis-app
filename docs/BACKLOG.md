...
- [ ] **OPEN — full spec-vs-code audit 2026-07-12** (5-agent code audit; supersedes the thinner "deferred" list that was here — measured against the whole spec, roughly build-order steps 8–18 are open). Grouped by build-order step:
  ...
  - [ ] **Step 7 leftovers — negative signals (§2.2) + per-user normalization (§2.3):** no skip penalty, no "not for me" hide control (no API subroute, no UI, no suppression, no decay), no category-level suppression, no per-user behavior baselines (median dwell/save-rate percentiles). Session `seen` list is dedup only, resets per session. **Cheapest high-leverage miss in the live feed.**
