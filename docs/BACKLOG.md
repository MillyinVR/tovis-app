...
- [ ] **OPEN — full spec-vs-code audit 2026-07-12** (5-agent code audit; supersedes the thinner "deferred" list that was here — measured against the whole spec, roughly build-order steps 8–18 are open). Grouped by build-order step:
  ...
  - [ ] **Step 10 — diversity injection (§4.3.1) + session intent (§4.3.2) + composition ratio (§4.3):** feed has NO inspiration/bookable distinction or ratio, no reserved exploration share, no intent shifting (entry point classified for logging only — `resolveDefaultCohort`).
