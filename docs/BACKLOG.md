...
- [ ] **OPEN — full spec-vs-code audit 2026-07-12** (5-agent code audit; supersedes the thinner "deferred" list that was here — measured against the whole spec, roughly build-order steps 8–18 are open). Grouped by build-order step:
  ...
  - [ ] **Step 13 — post-booking relationship layer (§6.7):** booking creates NO feed relationship (follow boost 25 exists but booking≠follow); "your pro has an opening" only via last-minute openings/priority offers (pro-triggered, waitlist-based, not cadence-aware); rebook window is static per-offering `rebookIntervalDays`; **`lib/clients/relationshipIntelligence.ts` already computes learned per-pair `cadenceDays` + `retentionRisk` — the §6.7 primitive — but is pro-facing informational only, wired to nothing.** Reviews/rebooks feed no ranking.
