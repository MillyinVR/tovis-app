...
- [ ] **OPEN — full spec-vs-code audit 2026-07-12** (5-agent code audit; supersedes the thinner "deferred" list that was here — measured against the whole spec, roughly build-order steps 8–18 are open). Grouped by build-order step:
  ...
  - [ ] **Step 2/§9 — metrics tail:** save→book funnel, board-creation→first-booking, saved-not-booked gap, feed-freshness %, hide-rate, notification opt-out-rate per trigger, platform-wide rebook rate. (What exists: `logLooksFeedServe` structured serve logs; per-look booking attribution; per-pro retention proxies in `proMonthlyAnalytics`.)
