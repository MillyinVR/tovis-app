...
- [ ] **OPEN — full spec-vs-code audit 2026-07-12** (5-agent code audit; supersedes the thinner "deferred" list that was here — measured against the whole spec, roughly build-order steps 8–18 are open). Grouped by build-order step:
  ...
  - [ ] **Step 9 — bookable-score terms (§4.2/§4.5), all absent:** `availability_boost` (needs a per-pro next-opening/calendar-fullness primitive — deliberately omitted from both feeds today, see `boardFeedRanking.ts` header), `booking_conversion_rate` (bookings-from-look / saves — `Booking.sourceLookPostId` attribution exists, no funnel), `underbooked_pro_boost`, `pro_reliability` (cancel/no-show/response/rebook — data partially exists operationally, nothing aggregates into ranking), learned `price_fit`/`proximity_fit` (nothing learned today; search radius is explicit user setting, `maxPrice` is a hard filter — spec wants soft weights).
