...
- [ ] **OPEN — full spec-vs-code audit 2026-07-12** (5-agent code audit; supersedes the thinner "deferred" list that was here — measured against the whole spec, roughly build-order steps 8–18 are open). Grouped by build-order step:
  - [x] **Step 8 — Badge system (§5) + commitment tiers (§5.3) + selection/rotation/TTL (§5.4–5.5, §5.7)** — **MERGED #602** (`924f7218`, 2026-07-12; branch deleted; iOS parity note tovis-ios #112). NOT yet deployed — next deploy applies migration `20260721000000` + the new hourly `pro-badge-stats` cron. Ships the engine (`lib/looks/badges/*`: evaluate → tier filter → §5.4 event override → daily rotation → sticky 5% §9 holdout), 7 live-data badge kinds, `ProfessionalBadgeStat` + hourly `pro-badge-stats` cron (migration `20260721000000`, applies next deploy), feed DTO `badge` field + card pill, optional `viewerLat/Lng` feed params, and badge-serve logging (eligible/shown/holdout/kind counts — the §9 scaffolding landing WITH its first consumer). Deferred from this step: impression capping (§4.6), board-feed + grid-tile badging, availability-class badges (needs the step-9 primitive), iOS pill render (tovis-ios #112).
  - [x] **Step 9 — bookable-score terms (§4.2/§4.5): COMPLETE 2026-07-15 (every term done).** `availability_boost` **DONE** (`feat/algo-availability-primitive`, **PR #604**, 2026-07-12): new `ProfessionalAvailabilityStat` (@id professionalId) + additive migration `20260723000000` (no flag, applies next deploy; zero migrate-diff drift) + hourly `pro-availability-stats` cron (`25 * * * *`) → `refreshProfessionalAvailabilityStats` computes per-pro **next opening + 14-day fullness + open-day count** from REAL working hours vs bookings/holds/blocks (primary bookable schedule; minute-level, coarse-by-design — NOT the slot engine). Soft `computeAvailabilityBoost` term (`availabilityMax 12 × (0.5·soon + 0.5·openness)`, 7-day soon half-life) reused by **both** the personalized + board re-rank (guardrail #8 — never a hard filter); reader `fetchProAvailabilitySignals` (one PK read); `availabilitySignalCount` on both metas + `looks_feed_serve`. DARK until the cron populates the table (feeds byte-identical). Only pros with a horizon opening get a row (skip-the-zeros). iOS parity = server-only (no PR). **Still open in step 9:** learned `price_fit`/`proximity_fit` (nothing learned today; search radius is explicit user setting, `maxPrice` is a hard filter — spec wants soft weights). The deferred **availability-class badge** (step 8) is now **DONE** (`feat/algo-availability-class-badge`, **PR #626**, MERGED 2026-07-15 `26eb83ad`): two new `LookBadgeKind`s off `ProfessionalAvailabilityStat` read in `lib/looks/badges/attach.ts` (5th bounded IN-list query, folded onto `ProBadgeSignals.availability`) — **`AVAILABLE_SOON`** (CONVENIENCE/info, `nextOpeningDate` ≤ 7d → "Available today/tomorrow/in N days") + **`BOOKING_OUT`** (URGENCY/warn, row present AND `fullness14d` ≥ 0.8 → "Almost booked out"). The CONVENIENCE/URGENCY split lets the existing §5.3 guard suppress ONLY the scarcity read on HIGH-commitment (permanent makeup keeps AVAILABLE_SOON, drops BOOKING_OUT); §5.7.4 TTL = 6h against the stat's OWN hourly-cron `computedAt`. NO migration/flag/cron (primitive+cron shipped in #604), additive schema (2 enum values), DARK until the availability cron populates the table; card renders by tone+label generically so no client/route change; iOS = server-only. `underbooked_pro_boost` is now **DONE** (`feat/algo-underbooked-pro-boost`, **PR #628** MERGED 2026-07-15 `4dc52d48`): a fairness / on-ramp additive term on the personalized re-rank (TikTok new-creator velocity analog, tied to calendar health) so discovery doesn't concentrate on already-busy pros. Pure `computeUnderbookedProBoost` (`underbookedMax 10 × clamp(1 − completedBookingCount30d / underbookedFullBookings 8, 0, 1)`) — 0 completed → full lift, tapering to 0 exactly at the "N bookings in 30 days" social-proof badge bar (`booked30dMin` 8); **GATED on a real near-term opening** (reuses the #604 `availabilitySignals` map presence as a calendar-health floor — NO extra read for the gate, and it filters dead/unbookable accounts). Deliberately NON-overlapping with `availability_boost`: that grades on FORWARD openness, this on BACKWARD completed-booking VOLUME; the shared "has an availability row" is a binary bookability floor both require, not a doubled weight. Signal read via a new `fetchProUnderbookedSignals` serve reader off `ProfessionalBadgeStat.completedBookingCount30d` (one bounded IN-list, parallel with the availability read; absent row = 0 completed = maximally under-discovered, so a brand-new pro with no badge-stat row earns the FULL on-ramp). `underbookedBoostedCount` on the personalized meta + `looks_feed_serve` (the §9 "is the floor reaching new/underbooked pros" metric). Off unless the caller wires `underbookedSignals` → board feed UNCHANGED (purpose-narrow, like #605 diversity). NO migration/flag/cron/notif (reuses the existing hourly `pro-badge-stats` + `pro-availability-stats` crons) → byte-identical until both crons populate; a pure ranking change, merges when green. iOS parity = server-only. `booking_conversion_rate` is now **DONE** (`feat/algo-booking-conversion-rate`, **PR #629 MERGED 2026-07-15 `596f4fca`**; the first step-9 FUNNEL/quality term — needed a NEW aggregate + cron, heavier than #628 which reused existing aggregates): the persisted `rankScore` blends likes/saves/comments/shares but NEVER bookings, so a "pretty" look (heavily saved, rarely booked) can win on engagement alone; per spec §4.2 (`bookings_from_this_look / (saves + remix_clicks)`) this rewards "content that fills chairs." NEW per-LOOK aggregate `LookPostConversionStat` (`lib/looks/conversionStats.ts`; additive migration `20260730000000`, zero migrate-diff drift) + hourly `look-conversion-stats` cron (`40 * * * *`, offset from the `10 *` pro-badge / `25 *` availability §4.2 siblings): ONE grouped SQL join over Booking→LookPost — attributed non-cancelled bookings via `sourceLookPostId` (numerator) + the look's `saveCount + viewCount` snapshot (interest denominator; views stand in for the untracked remix-click signal). Grouping over bookings means ONLY converting looks get a row (skip-the-zeros is automatic, table stays sparse); only PUBLISHED+APPROVED source looks counted. Pure `computeBookingConversionBoost` (`personalizedRanking.ts`): Bayesian-smoothed bookings/(saves+views) → `conversionMax 8 × clamp(smoothedRate / conversionTargetRate 0.04, 0, 1)`, prior `{rate 0.01, strength 40}`, **GATED on bookingCount > 0** (no attributed booking → 0, NOT a prior baseline). Peak below availability (12) / category cap (15) / follow (25) / booked (30) — a soft quality tie-breaker. Deliberately ORTHOGONAL to `underbooked` (a pro's booking VOLUME) + `availability` (forward openness): this grades THIS LOOK's booking-per-exposure EFFICIENCY, so a new pro's high-converting look earns both (no double-count; and rankScore never counts bookings). Reader `fetchLookConversionSignals` + structural `LookConversionReaderDb` (bounded IN-list by look id, parallel with the availability/underbooked reads, for the re-ranked candidate set only). Optional `conversionSignals` on `PersonalizedRankContext` → board feed / omitting callers byte-identical. `conversionBoostedCount` on the personalized meta + `LooksFeedServeEvent` + route serve-log. NO flag/client surface/notif — a pure ranking change + one new infra cron, DARK until the cron populates; iOS = server-only; merges when green (a cron is infra, not a notif surface, like #625/#626/#628). `pro_reliability` is now **DONE** (`feat/algo-pro-reliability`, **PR #630**; the step-9 TRUST/quality term — CHEAPER than #629 because it EXTENDS the existing `ProfessionalBadgeStat` + hourly `pro-badge-stats` cron, no new table/cron): the persisted `rankScore` + all §4.2 terms so far grade demand/openness/efficiency but nothing grades whether a pro SEES BOOKINGS THROUGH; per spec §4.2 ("boosting a pro who cancels burns the client's trust in the platform") a high completed-vs-cancelled FOLLOW-THROUGH rate earns a modest per-pro lift. TWO additive columns on `ProfessionalBadgeStat` (`resolvedBookingCount` = COMPLETED + CANCELLED scheduled in the trailing 180d; `completedResolvedCount` = the COMPLETED subset — migration `20260731000000`, zero migrate-diff drift) filled by a **4th grouped Booking query** in `refreshProfessionalBadgeStats` (the existing hourly `10 *` cron — no new cron). **NO_SHOW is DELIBERATELY excluded** from both counts (client behaviour, not the pro's reliability — must not down-rank the pro); cancellations are NOT split by initiating party (attribution lives only in notification event keys, not on Booking), so this is a booking-completion proxy kept a modest, smoothed soft weight per the guardrail. Pure `computeProReliabilityBoost` (`personalizedRanking.ts`): Bayesian-smoothed completion rate → `reliabilityMax 6 × clamp((smoothedRate − reliabilityFloorRate 0.75) / (1 − 0.75), 0, 1)`, prior `{rate 0.9, strength 10}`, **GATED on resolvedBookingCount > 0** (no resolved bookings → 0, NOT a prior baseline). `reliabilityMax 6` = the SMALLEST bookable term (below conversion 8) per the "modest, never a directory sort" guardrail (over-penalizing a pro for CLIENT-side cancels burns trust the other way). Deliberately ORTHOGONAL: underbooked = booking VOLUME, availability = FORWARD openness, conversion = a LOOK's efficiency, REBOOK_RATE badge = loyalty (do clients return); this grades FOLLOW-THROUGH (do committed appointments happen). NOT availability-gated (a trust signal, like conversion). New `fetchProReliabilitySignals` reader + structural `ProReliabilityReaderDb` (own bounded IN-list off `ProfessionalBadgeStat`, parallel with the underbooked/availability reads in ONE `Promise.all` — same table as underbooked but independently gated so each §4.2 term stays byte-identical alone). Optional `reliabilitySignals` on `PersonalizedRankContext` → board feed / omitting callers byte-identical. `reliabilityBoostedCount` on the personalized meta + `LooksFeedServeEvent` + route serve-log. NO flag/client surface/notif/new-cron — a pure ranking change + additive columns on an existing aggregate, DARK until the existing cron repopulates; iOS = server-only; merges when green (like #625/#626/#628/#629). **Closes the §6.7 "how did it go?" outcome loop into ranking.** `price_fit` is now **DONE** (`feat/algo-price-fit`, **PR #631**; the LAST §4.5 term, saved for last because it is the FIRST learned per-USER signal — not a per-pro/per-look aggregate): nothing was learned about a client's budget — search radius is an explicit user setting and `maxPrice` a HARD filter, but spec §4.5 wants a SOFT weight ("surfacing $400 colorists to someone who books $60 services instantly breaks the knows-me illusion … a WEIGHT, not a filter; aspirational saving is real"). Learns a per-user PRICE BAND from the client's completed bookings — folded into the EXISTING §6.7 booking read (`fetchClientBookingSignals`, `relationshipSignals.ts`; NO new query): a new `resolveBookingServicePrice` resolves each booking's service price (`serviceSubtotalSnapshot ?? subtotalSnapshot`, the DTO's rule) onto `CompletedBookingSignalRow.servicePrice`. Pure `learnPriceBand` (`personalizedFeed.ts`) → `LearnedPriceBand {logCenter, sampleCount}`: the RECENCY-weighted mean of ln(servicePrice) (LOG space — price perception is multiplicative; each visit weighted by age at the slow booking half-life `AFFINITY_BOOKING_HALF_LIFE_DAYS` 180d, the same decay the §2 category fold uses), plus a count of priced bookings for the confidence ramp. `null` when no booking carries a usable price → term dark. Pure `computePriceFitBoost` (`personalizedRanking.ts`): `priceFitMax 8 × fit × confidence` where `fit = exp(−(ln price − logCenter)²/(2 σ²))` (Gaussian in log space, `priceFitLogSigma` 0.9 → ~0.74 of peak at 2× off-center, ~0.30 at 4× — gentle, never a cutoff) and `confidence = clamp(sampleCount / priceFitFullBookings 3, 0, 1)`; **GATED** on a learned band + a positive look price (`LookPost.priceStartingAt`, already on `looksFeedSelect`). `priceFitMax 8` sits in the bookable-term band (== conversion), below taste (cap 15)/follow (25)/booked (30) — a tie-breaker, never a directory sort. Band lives on `PersonalizedViewerAffinity.priceBand` (viewer state like `relationshipSignals`), read off the row like `categoryBoost`; structural `{toNumber}` price type keeps Prisma out of the pure ranker (`resolveLookPriceNumber`). `priceFitBoostedCount` on the personalized meta + `LooksFeedServeEvent` + route serve-log (a COVERAGE metric — priced looks scored against a band; the Gaussian never zeroes, ordering buries far-out looks). NO migration/flag/cron/notif/new-table — a pure ranking change reusing the §6.7 read → byte-identical for a viewer with no priced bookings; board feed UNCHANGED; iOS = server-only; merges when green (like #625/#626/#628/#629/#630). Deliberately GLOBAL-per-user v1 (not spec §4.5's per-CATEGORY band — too sparse with few bookings; the global band already solves the stated "$400-to-a-$60-client" failure). `proximity_fit` is now **DONE** (`feat/algo-proximity-fit`, **PR #632 MERGED 2026-07-15 `8e471f3a`**; the LAST §4.2/§4.5 bookable term — completes step 9): nothing was learned about how far a client will travel — search radius is an explicit user setting and `maxPrice` a HARD filter, but spec §4.5 wants a SOFT distance weight ("surfacing pros 40 minutes away instantly breaks the knows-me illusion … a WEIGHT, not a filter; inspiration content is fine from anywhere"). Deliberately the **request-param v1** (the spec's "default to a metro-sensible radius pre-data" prong), avoiding the heavier stored-centroid / booking-location-PII path: the viewer "here" is the request's optional, range-checked `viewerLat/Lng` (parsed once up front in the route, now shared with the §5 DISTANCE badge), NOT a learned/stored geo center. New serve reader `fetchProProximitySignals` (`lib/looks/proximityStats.ts`) reads each displayed pro's PRIMARY, non-archived `ProfessionalLocation` coordinate (one indexed IN-list, same coords the DISTANCE badge already reads — a pro's BUSINESS location, not client PII) and computes the viewer→pro `haversineMiles`, returning a **plain distance** (`ProProximitySignal {distanceMiles}`) so the pure ranker never touches geo/Prisma (mirrors `ProAvailabilitySignal`); structural `ProProximityReaderDb` keeps it unit-testable. Pure `computeProximityFitBoost` (`personalizedRanking.ts`): `proximityFitMax 8 × fit`, `fit = exp(−miles²/(2 σ²))` — a Gaussian in MILES centered at the viewer's doorstep, `proximityFitSigmaMiles 8` (the spec's metro-sensible default radius: ~0.61 of peak at 8mi, ~0.01 at 24mi, ≈0 by ~40 min out). NO confidence ramp (the viewer's current location is a fact when provided, not accrued, unlike the learned price band); GATED on a present signal (pro has a coord AND the request carried coords). `proximityFitMax 8` sits alongside `price_fit` in the bookable band, below availability (12)/taste (15)/follow (25)/booked (30) — a tie-breaker, never a directory sort. `proximitySignals` on `PersonalizedRankContext` (optional, per-pro map, joins the same `Promise.all` batch); the pro-location read is SKIPPED entirely when the request carried no viewer location → the term is off, byte-identical. `proximityFitBoostedCount` on the personalized meta + `LooksFeedServeEvent` + route serve-log (a COVERAGE metric — the Gaussian never zeroes, so ordering, not this count, buries far pros). Guard notes: `check:tenant-aware-discovery` passes (`professionalLocation.findMany` only counts as a discovery surface WITH `PUBLICLY_APPROVED_PRO_STATUSES`, which this doesn't reference — reads an already-tenant-scoped pro set, like `attach.ts`); `check:pii-plaintext-reads` passes (the `lat/lng` select is INLINE, not on its own line, the established `attach.ts` convention for pro business coords). NO migration/table/cron/flag/notif → byte-identical for a request with no viewer location; board feed UNCHANGED; iOS = server-only; merged when green (like #625/#626/#628/#629/#630/#631). Deferred v2 (NOT this build): a LEARNED per-user travel radius (widen for a bridal board, learn from booking-location history) — same "global v1 first" pattern price_fit used before per-category. **Step 9 is now COMPLETE — every §4.2/§4.5 bookable-score term (availability, underbooked, conversion, reliability, price_fit, proximity_fit) is done.**
  - [~] **Step 7 leftovers — negative signals (§2.2) + per-user normalization (§2.3):** **explicit "not for me" hide control DONE** (`feat/algo-hide-control`, 2026-07-12): `LookHide` model + migration `20260722000000` (additive, applies next deploy), idempotent `POST/DELETE /api/v1/looks/[id]/hide`, one-tap `EyeOff` control on the feed card (optimistic removal + login-bounce for guests), hard exclusion of hidden looks from the personalized + chronological + board feeds, and a DECAYED category-level suppression (30-day half-life, past a repeated-hide threshold; guardrail #10) in the personalized re-rank. Hide counts (`hiddenExcludedCount`/`categorySuppressionCount`) added to `looks_feed_serve` (§9 early-warning). iOS parity deferred (tovis-ios/BACKLOG.md §4). **Still open in §2.2/§2.3:** inferred *skip* penalty (needs dwell/scroll-speed capture — not tracked yet), board-feed inferred-skip category suppression, and per-user behavior baselines (median dwell/save-rate percentiles, §2.3). Session `seen` list is still dedup-only.
  - [x] **Step 10 — diversity injection (§4.3.1) + session intent (§4.3.2) + composition ratio (§4.3)** — **DONE** (`feat/algo-diversity-injection`, **PR #605**, 2026-07-12). Pure `lib/looks/feedComposition.ts` (unit-tested): `SessionIntent` (`default`/`book`/`dream`) → `resolveCompositionPlan` (intent lean + reserved-exploration size) + `interleaveExploration` (evenly-spaced, ON TOP, never displaces the backbone → cursor untouched). Three soft/additive mechanisms on the personalized feed: **(a) §4.3.2 session intent** — optional `?intent=` request hint (opened from an opening push → `book`; cold → `default`; search stays on the non-personalized path), absent→`default`; **(b) §4.3 composition ratio** — intent scales the `availability_boost` weight (new `weightMultiplier` on `computeAvailabilityBoost`: book ×1.75 leans bookable, dream ×0.5 leans inspiration, default ×1) so "bookable-now" leans on the #604 availability primitive; the displayed bookable/inspiration split is reported (`bookableCount`/`inspirationCount` on the meta, dark until the cron); **(c) §4.3.1 diversity injection** — reserves ~12% of the entry page (`round(limit × share)`, capped 4) for high-quality content OUTSIDE the viewer's affinity categories (nested `service.category.slug notIn`) + off followed pros, ranked by global rankScore, gated on a confident graph (≥4 affinity categories) AND the new **`ENABLE_FEED_DIVERSITY_INJECTION`** env flag (default OFF → byte-identical deploy). New meta (`sessionIntent`/`availabilityWeightMultiplier`/`explorationInjectedCount`/`bookableCount`/`inspirationCount`) added to `looks_feed_serve`. Board feed deliberately UNCHANGED (purpose-narrow — diversity injection would fight its intent; `computeAvailabilityBoost` defaults multiplier 1 there). NO migration; iOS parity = server-only (no PR — the client just gets the default lean; iOS can send `?intent=book` from an opening push later). Tests: `feedComposition.test.ts` (17 units), ranking multiplier units, feed composition/exploration units, observability field units, real-Postgres `tests/integration/feed-diversity-injection.test.ts` (nested-relation `notIn` exclusion + null-service drop). **Still open in §4.3:** §4.3.2 in-session behavior escalation (needs the client to re-request with a stronger hint on availability/pricing taps — hook exists, no UI wiring yet), §4.7 small-market degradation order (widen radius → relax taste → raise inspiration share), turning the flag on (Tori's call).
  - [x] **§4.6 impression capping & freshness — COMPLETE 2026-07-15 (both halves).** **freshness ratio + retrieval widening DONE** (`feat/algo-freshness-widening`, **PR #634 MERGED** `d63d4bbf`): the personalized feed's backbone/injection/exploration all hard-exclude the session seen + hidden sets, so those items are never-seen by construction and the fresh supply "runs out" exactly when the assembled page is short of the requested `limit`. New per-serve **`freshnessRatio`** = `clamp(freshCount / limit, 0, 1)` (the spec §4.6 "% of cards the user has never seen" / §9 feed-freshness metric — computed always) + **retrieval widening**: when the fresh page is short AND a session seen set exists, backfill the remaining slots with the globally-best PREVIOUSLY-SEEN looks (hidden stay hard-excluded; this page's ids too), appended AFTER the fresh items so never-seen content always leads — rather than serve a short/empty page (spec §4.7 "an empty feed is worse than a loosely-matched one"). Only ever fires on the TERMINAL page (a short fresh backbone ⇒ `hasMore` false ⇒ `nextCursor` null), so no infinite re-show; a cold entry (no seen set) stays byte-identical (no extra query). `freshnessRatio` + `widenedBackfillCount` on the personalized meta + `LooksFeedServeEvent` + route serve-log. Pure query-composition (reuses the backbone's `id notIn` semantics), NO migration/table/cron/flag/notif. **per-user-per-look impression cap DONE** (`feat/algo-impression-cap`, **PR OPEN 2026-07-15**; the deferred heavier half): the session `seen` list dedupes WITHIN a session but is client-supplied + resets, so nothing stopped the feed re-surfacing the same look session after session. Now a PERSISTENT per-(viewer, look) FEED-exposure counter caps a look out of the personalized feed after `IMPRESSION_CAP_EXPOSURES` (4) exposures (it can still be opened directly — the cap only governs the FEED). NEW `LookViewerImpressionStat` (`@@id([userId, lookPostId])`, count + lastSeenAt; additive migration `20260801000000`, zero migrate-diff drift) written by the EXISTING `APPLY_LOOK_VIEWS` job — the views route now resolves the session (`getOptionalUser`, guest-allowed so guests omit the viewer + only the aggregate viewCount/§5.6 path runs, byte-identical) and threads a server-set `viewerId`; `processApplyLookViews` bumps the counter for each eligible **FEED** impression only (DETAIL = explicit nav, never counts; pure `selectViewerCappedLookIds`). Reader `lib/looks/viewerImpressionCap.ts` (`loadCappedLookIds`, structural DB type, mirrors `hides.ts`) folds into `loadPersonalizedAffinity` keyed by userId; capped ids join the hard-exclusion set (backbone + injection + exploration AND the widening backfill — a capped look "reappears only via a state change / explicit nav", never re-shown). `cappedExcludedCount` on the personalized meta + `LooksFeedServeEvent` + route serve-log (pair with `freshnessRatio` to catch a drying-up graph). NO flag/notif; a pure ranking + one additive table (a cron is not involved — the writer is the existing view-ingest job), DARK until per-viewer counters accrue; board feed UNCHANGED; iOS = server-only (identity resolved server-side, no client change); merges when green (like #628–#632/#634). **Still deferred (smaller §4.6 nuances):** the "a couple more exposures if the look's badge state meaningfully changed" refinement (the cap exclusion runs BEFORE badge attach, so it needs a different structure); long-term `LookViewerImpressionStat` row retention/pruning (v1 keeps rows forever so time never re-shows a capped look, per the spec's "state change / nav only"); and coupling freshness→widening into the §4.7 small-market degradation order.
  - [~] **Step 12 — notification budget (§8.1) + saved-not-booked activation (§6.8) + §8 event countdown:** **budget + first §6.8 consumer + §8 event-date countdown ALL DONE & DEPLOYED 2026-07-15** (#620 + #621). **§8 event countdown — `feat/algo-event-countdown-notifications`, PR #621:** new `EVENT_DATE_COUNTDOWN` event key (client-only, IN_APP+EMAIL+PUSH, non-transactional) + "Event countdowns" preference category + additive migration `20260727000000` (zero migrate-diff drift) mapped to the reserved top-priority `EVENT_COUNTDOWN` budget trigger; consumer `lib/notifications/eventCountdownNotifications.ts` + daily cron `/api/internal/jobs/event-countdown` `5 10 * * *` (scheduled EARLIER than saved-look `20 10` so the top-priority countdown claims the pooled budget first — design decision (a); the (b) unified per-user dispatcher is the follow-up). Fires at 30/14/7/3-day milestones on dated bridal/prom boards (`Board.eventDate`, #511), deduped per (board, milestone), budget-gated + muted-skipped, white-label copy, `@db.Date` never touched by raw `toLocale*`. **budget + first §6.8 consumer DONE** (`feat/algo-notification-budget-saved-activation`, **PR #620**, 2026-07-15). Shared §8.1 pooled weekly cap (`lib/notifications/reEngagementBudget.ts`, pure/unit-tested: trigger taxonomy + priority ladder [countdown > availability-opened > rebook > archive > other], `RE_ENGAGEMENT_WEEKLY_CAP` 3 over 7-day rolling window, all triggers pooled; ledger = existing `NotificationDispatch` count by re-engagement eventKey — NO new budget table; per-trigger mute is free via the per-`(client,eventKey)` preference row). First live consumer = §6.8 availability-opened-on-a-save (`lib/notifications/savedLookActivation.ts` + daily cron `/api/internal/jobs/saved-look-activation` `20 10 * * *`): new `SAVED_LOOK_AVAILABILITY_OPENED` event key (client-only, IN_APP+EMAIL+PUSH, non-transactional) + "Saved looks" preference category + additive migration `20260726000000` (applies next deploy, zero migrate-diff drift). Pro-anchored scan of aging saves (3–60d) whose pro now has a near-term opening (#604 `nextOpeningDate` ≤14d), excluding already-booked pairs + already-nudged (cooldown-bucketed dedupeKey), budget-gated + priority-ordered, muted recipients skipped (opt-out signal), honest/non-urgent/white-label copy. `reengagement_activation_serve` log = opt-out + budget-pressure metrics. Server-only (iOS N/A); NO flag; NO deploy. **§6.8 HESITATION blocker response DONE** (`feat/algo-hesitation-consult-nudge`, **PR OPEN 2026-07-15**; the 2nd §6.8 blocker + 4th pooled trigger): new `SAVED_LOOK_CONSULT_NUDGE` event key (client-only, IN_APP+EMAIL+PUSH, non-transactional) + "Consult reminders" preference category + additive enum migration `20260729000000` (zero migrate-diff drift) + `HESITATION_CONSULT` budget trigger (LOWEST live priority — no clock, so it yields the last pooled slot to all three time-sensitive triggers) + gen:api-schema. Consumer `lib/notifications/hesitationConsultNudge.ts` + daily cron `/api/internal/jobs/hesitation-consult` `40 10` + a 4th arm in the unified dispatcher (`gatherConsultNudgeCandidates`). Scans aging (5–60d, 45-day cooldown) saves on HIGH/MEDIUM-commitment looks — new `isConsultWorthyCommitmentSlug`/`consultWorthyCommitmentSlugs` in `lib/looks/badges/commitmentTiers.ts` reuse the §5.3 SSOT (excludes routine LOW + uncategorized) — whose pro the client NEVER booked, and sends ONE gentle, information-first "have questions? book a consult when you're ready" nudge (never urgency/scarcity — guardrail #2/§5.3), budget-gated + muted-skipped + cooldown-bucketed dedupeKey, pro name via the LookPost→professional relation (never a top-level pro findMany). Category-anchored scan (no availability gate — a consult can precede an opening). Server-only (iOS N/A); NO flag; NO deploy. **§6.8 PRICE blocker response DONE** (`feat/algo-price-alternative-nudge`, **PR #636 OPEN 2026-07-16**; the 3rd §6.8 blocker + 5th pooled trigger — the FIRST retrieval arm): new `SAVED_LOOK_PRICE_ALTERNATIVE` event key + "Similar looks in your range" preference category + additive enum migration `20260802000000` (zero migrate-diff drift) + gen:api-schema + `PRICE_ALTERNATIVE` budget trigger (lowest live priority — clockless like the consult nudge, and the more speculative §6.8 nudge since it steers to a DIFFERENT pro; above the reserved BOARD_ARCHIVE). Consumer `lib/notifications/savedLookPriceAlternative.ts` + daily cron `/api/internal/jobs/saved-look-price-alternative` `45 10` + a 5th arm in the unified dispatcher (`gatherPriceAlternativeCandidates`). Scans aging (5–60d, 45-day cooldown) PRICED saves priced ≥~3× (`ln 3`) ABOVE the client's LEARNED price band (§4.5 `learnPriceBand`, REUSED via a batched completed-booking band reader; band trusted only ≥3 priced bookings — mirrors `priceFitFullBookings`) whose pro the client NEVER booked, then RETRIEVES the best same-category, IN-band (≤~1.5× center), DIFFERENT-pro look — a tenant-scoped `lookPost.findMany` per tenant composing `proDiscoveryVisibilityFilter` (surfacing a NEW look is Pro discovery → `check:tenant-aware-discovery` passes; a client never sees another tenant's look), rankScore-ordered — and sends ONE gentle nudge linking to that alternative. Copy is PRICE-INVISIBLE (never names price/budget/cheaper — the band is selection logic, not a judgment); names the alt pro, anchors on the style the client liked. Why price and not too-far first: both retrieval arms need a learned signal, but `proximity_fit` (#632) is request-param-only so a CRON has no viewer location; the price band is derivable per-user from completed bookings inside the cron. Consolidated the now-triplicated `loadBookedPairs`+statuses into `reEngagementLedger.loadBookedReEngagementPairs` (savedLookActivation + hesitation now share it, behavior-preserving — their integration tests re-run green). Tests: 18 pure units + real-Postgres integration (aging-save scan → band learning → tenant-scoped discovery → emit; band/booked exclusions; idempotent). DARK until priced bookings accrue + crons run; server-only (iOS N/A); NO flag; NO deploy — left OPEN for Tori (copy tone + marketplace steering-to-a-different-pro are her call, like #620–#624). **Still open in step 12:** the **too-far** §6.8 blocker (needs a STORED viewer geo center — the deferred v2 half of #632; proximity is request-param-only, so a cron has no viewer location, unlike the learned price band — build the stored/learned centroid first); just-dreaming → leave alone, no work. Opt-out-RATE aggregation (numerator captured; the metric consumer is the §9 tail).
  - [~] **Step 13 — post-booking relationship layer (§6.7):** **feed prong DONE** (`feat/algo-relationship-boost`, **PR #606**, 2026-07-12). A booking is the strongest signal in the hierarchy (§2) yet ranking wired nothing off it; now booked-pro looks surface in that client's personalized feed. NEW `lib/looks/relationshipSignals.ts` (`fetchClientRelationshipSignals` + pure `aggregateRelationshipSignals`: viewer's COMPLETED bookings → per-pro {latest visit, visit count}, one client-scoped query) + `computeRelationshipBoost` (`relationshipMax 30 × clamp(0.5·recency + 0.5·loyalty, 0, 1)`; recency half-life 120d = slowest of any signal, loyalty saturates at 3 visits; ADDITIVE like follow, so a lapsed-but-loyal pair keeps surfacing = the feed side of the §6.7 re-engagement moment). Loaded once per request in `loadPersonalizedAffinity` (client-gated), on `PersonalizedViewerAffinity.relationshipSignals`. New meta `relationshipProCount`/`relationshipBoostedCount` (§9 on-platform rebook metric). NO migration, NO flag — empty map → boost 0 → byte-identical; board feed UNCHANGED; iOS parity server-only. **Cadence-timed rebook prompts (notification prong) DONE** (`feat/algo-rebook-cadence-notifications`, **PR #622**, MERGED 2026-07-15 `486c5832`; NOT yet deployed — landed after the 2026-07-15 prod deploy): new `REBOOK_CADENCE_DUE` event key + additive enum migration `20260728000000`, consumer `lib/notifications/rebookCadenceNotifications.ts` + daily cron `/api/internal/jobs/rebook-cadence` `35 10` reading LEARNED per-pair cadence (mean consecutive completed-visit gap, `computeMeanCadenceDays`) falling back to the offering's `rebookIntervalDays`, due window [cadence, 3×cadence], requires a #604 near-term opening, budget-gated + muted-skipped; refactor extracted the shared budget/opt-out/dedupe/open-pro reads into `lib/notifications/reEngagementLedger.ts` (net −192 lines). Now allocated GLOBALLY by the §8.1 unified dispatcher (below). **Booking→CATEGORY taste graph DONE** (`feat/algo-booking-category-affinity`, **PR OPEN 2026-07-15**): a completed booking — the strongest §2 signal — now lifts the client's affinity for that service CATEGORY, not just the per-pro boost. Folded into `loadPersonalizedAffinity` alongside likes/saves via a new pure `bookingCategoryAffinityEntries` (`AFFINITY_BOOKING_WEIGHT` 4 = heaviest behavioral source, decayed at the slowest half-life `AFFINITY_BOOKING_HALF_LIFE_DAYS` 180d; the ranker's `categoryWeightCap` 5 keeps one category from burying the feed). Consolidated the existing §6.7 completed-booking read: `fetchClientRelationshipSignals` → `fetchClientBookingSignals` returns `{ relationshipSignals, completedBookings }` from ONE bounded query (added `service.category.slug` to the select), so the relationship map + category fold share a single read (no 2nd booking scan). Reuses `computeAffinityDecayFactor` + the shared `completedVisitInstant` (finishedAt ?? scheduledFor). NO migration, NO flag, byte-identical when the client has no completed bookings; iOS parity = server-only. **Still open in §6.7:** the "your pro has a Thursday opening" first-class card (a UI surface, not a notification), the "how did it go?" outcome loop → `pro_reliability` (step 9); `relationshipIntelligence.ts` cadence/retention still not read by ranking.
  - [x] **§8.1 UNIFIED re-engagement dispatcher (the capstone)** — **PR #623 (MERGED 568c89d1, 2026-07-15; NOT yet deployed — flag `ENABLE_UNIFIED_REENGAGEMENT_DISPATCH` default OFF)** (`feat/algo-unified-reengagement-dispatch`, 2026-07-15). Fully realizes §8.1. The three re-engagement crons (event-countdown `5 10`, saved-look `20 10`, rebook-cadence `35 10`) shared the pooled budget LEDGER but arbitrated priority only WITHIN a single scan — cross-trigger priority was approximated by CRON ORDERING (design decision (a); documented in all three emitters). This ships the (b) alternative: ONE cron `/api/internal/jobs/re-engagement-dispatch` (`50 10`) that GATHERS every trigger's candidates and runs `allocateBudgetToCandidates` ONCE per user, so the ladder (countdown > availability-opened > rebook) is enforced GLOBALLY — a countdown wins a client's last pooled slot over a rebook even when the rebook was "found first" (the guarantee cron-ordering couldn't make). New `lib/notifications/reEngagementDispatcher.ts`: pure `allocateReEngagementDispatch` (per-eventKey mute drop → per-client global-priority pooled allocation, reusing `allocateBudgetToCandidates` + `RE_ENGAGEMENT_TRIGGER_PRIORITY`) + impure `runReEngagementDispatch` gathering all three via each consumer's NEW exported `gatherEventCountdownCandidates` / `gatherSavedActivationCandidates` / `gatherRebookCadenceCandidates` (a behavior-preserving extraction of the impure orchestrator bodies — the three `runX` fns now call the same gather). Reversible flag-gated cutover: `ENABLE_UNIFIED_REENGAGEMENT_DISPATCH` (`lib/notifications/reEngagementDispatchFlag.ts`, default OFF → byte-identical: the 3 per-trigger crons run exactly as today + the dispatch cron no-ops; ON → the dispatcher does all the work + the 3 per-trigger routes early-return, all four sharing the idempotent dedupeKey ledger so the cutover never double-sends). NO migration / NO new enum / NO api-schema change (reuses the 3 existing event keys). `reengagement_dispatch_serve` log = pooled mutedOptOut/budgetBlocked + per-trigger `candidatesByTrigger`/`sentByTrigger`. Server-only (iOS N/A); NO deploy — left OPEN for Tori (same caution as #620/#621/#622). Tests: 8 pure allocator units (global-priority-beats-found-first, full-ladder, pooled already-sent, within-tier urgency, per-trigger mute, independent budgets, empty) + flag unit + real-Postgres `tests/integration/re-engagement-dispatch.test.ts` (a §8 countdown beats a §6.7 rebook for the SAME client's last pooled slot on a real DB; rebook-only client still nudged; idempotent) + the 3 sibling integration tests re-run green (behavior-preserving). **Still open in §8.1:** the **too-far** §6.8 blocker response (price is now a 5th registered dispatcher arm, PR #636 OPEN 2026-07-16; too-far needs a stored viewer geo center — deferred v2 of #632); the §7.5 board-archive trigger (its priority tier is already reserved); opt-out-RATE aggregation (§9 tail); flipping the flag on (Tori's call).
  - [x] **Step 14 — §5.6 velocity-anomaly READER/check — DONE 2026-07-16** (`feat/algo-velocity-anomaly-check`, **PR #637**): the write-only `LookPostImpressionStat` capture (per-source, per-day) now has its READER + admin-review surface. New `lib/looks/velocityAnomaly.ts` — a PURE `evaluateLookVelocityAnomaly` + an impure `detectLookVelocityAnomalies(db, {now, windowDays, limit})` window scan — surfaces looks whose recent engagement looks gamed so a HUMAN can review the pro; it NEVER auto-penalizes (impressions are best-effort sampled, so a flag is a manual-review lead, not a verdict — exactly the spec §5.6 "flag … for manual review" posture). TWO cheap, independent reasons over a trailing daily window (default 7d, clamp 1–30): **`RATE_ANOMALY`** (the signal the impression table exists for) = window (saves+likes) outrunning window impressions — "fake saves without matching impressions produce impossible rates the anomaly check catches trivially" (§5.6 NEW / §4.1); `ratio = engagement / max(impressions,1) ≥ 1.5` with an 8-engagement floor (kills tiny-sample noise; you can't save what you never saw, and per-session impressions normally dwarf engagers). **`HISTORICAL_SPIKE`** = window engagement-per-day ≥ 5× the look's OWN pre-window daily rate ("engagement that spikes far outside a … normal historical pattern"), computed from the look's lifetime counts (no extra query), GATED on a real prior baseline (≥7 prior days AND non-zero prior engagement) so a new look's honest launch surge or a dormant look's first real discovery never false-positives — a dormant FAKE burst is still caught by RATE_ANOMALY. A finding can trip either/both; `severity` blends the two normalized sub-scores (both ≥1 when tripped) so a two-reason look floats to the top of the queue; spike multiple capped at 999 for a stable sort. Cost = 3 bounded windowed aggregates (top-500 engaged looks by saves + by likes, then their impressions) + one metadata `findMany` over the candidate set; drives from ENGAGEMENT (a look with impressions but no engagement can't be anomalous) and evaluates only feed-visible (PUBLISHED+APPROVED, cross-tenant via `platformCrossTenantProVisibilityFilter` — `check:tenant-aware-discovery` passes) looks, labelled with PUBLIC fields only (businessName/handle, never firstName → no PII boundary). Admin consumer: `GET /api/v1/admin/looks/velocity-anomalies` (SUPER_ADMIN or REVIEWER) + a read-only review page `app/admin/looks/anomalies` (window selector, severity-sorted cards, reason badges, "View look" / "Review pro" links) + an admin-home card. NO migration/enum/DTO (so NO gen:api-schema)/flag/cron/notif — a pure read over already-captured data; DARK-safe (empty until per-day impression + engagement rows accrue). Tests: 12 pure evaluator units (floor, rate ceiling inclusive/just-under, zero-impression strongest, spike vs prior baseline, no-baseline/new-look non-flag, cap, both-reasons ordering, input guards) + a real-Postgres round trip `tests/integration/look-velocity-anomaly.test.ts` (abusive flagged, healthy ignored, spike caught, unpublished excluded, severity-sorted). Pure observability/admin → merges when green (no notif surface); iOS = server-only. **Still deferred (§9 tail):** rolling the flagged pros into an automated alert / a per-pro rollup, and coupling the anomaly signal back into rank suppression (deliberately NOT done — manual review only, per spec).
  - [x] **Step 2/§9 — metrics tail — DONE 2026-07-16** (`feat/algo-personalization-metrics`, **PR #638** `591ce354`): the READER/rollup consumer for the funnel + health signals the ranking/re-engagement work has been emitting all along; companion to the §5.6 velocity-anomaly reader (#637) — same admin-route + read-only-page + admin-home-card shape, pure observability. New `lib/looks/personalizationMetrics.ts` — pure rate/derivation helpers (`safeRate`, `median`, `deriveSaveToBookFunnel`, `deriveRebookMetric`, `summarizeCategoryOptOuts`) + an impure `computePersonalizationMetrics(db, {now, windowDays})` window scan over EXISTING tables (default 30d window, clamp 7–90). Metrics, all DB-derivable: **save→book funnel + saved-not-booked gap** (distinct (client, look) pairs saved in the window, and how many the same client then booked via a look-attributed non-cancelled `sourceLookPostId` booking — the complement is literally the §6.8 nudge population); **board-creation→first-booking** (board creators in the window, share who then booked, + median days to it); **hide-rate** (window `LookHide` over window FEED `LookPostImpressionStat`); **notification opt-out per trigger** (per category, share of clients who muted every channel — reuses the re-engagement mute predicate + `getNotificationCategoriesForAudience('client')`; missing prefs = opted-in; a point-in-time snapshot, not windowed); **platform rebook rate** (of clients with ≥1 completed booking, share with ≥2 — lifetime retention). Deliberately NOT here: **feed-freshness %** and the per-serve boosted counts live only in the `logLooksFeedServe` structured serve logs (`lib/observability/looksFeedEvents.ts`, console-only — never persisted), so they're log-analytics not a DB rollup; the page says so rather than fake a proxy. Consumer: `GET /api/v1/admin/looks/personalization-metrics` (SUPER_ADMIN or REVIEWER) + read-only `/admin/looks/metrics` page (window selector, stat tiles, per-trigger opt-out table) + an admin-home card. All aggregates are guard-free counts/groupBys/aggregates — no `lookPost.findMany`, so `check:tenant-aware-discovery` passes without a helper (one gotcha: a doc comment literally containing that method name tripped the substring guard → reworded). NO migration/enum/DTO (no gen:api-schema)/flag/cron/notif — a pure read over already-captured data, DARK-safe (empty until rows accrue). iOS = server-only. Pure observability → merged when green (like #637), not left-open-for-Tori. Tests: 15 pure units + `tests/integration/personalization-metrics.test.ts` (real-Postgres before/after DELTA round trip over every metric). **Still deferred (§9 tail):** the serve-log-only ratios (freshness %, boosted counts) need a log-drain rollup if they're ever wanted in-app; coupling any of these into an automated alert.
  - [x] **Step 15 — pro transparency dashboard (§6.5) — DONE 2026-07-16** (`feat/algo-pro-visibility-dashboard`, **PR #643**; iOS parity **tovis-ios PR #144**): the pro dashboard answered "what happened" (`creatorAnalytics.ts` — views/saves/followers/top looks); §6.5 asks the other question ("pros will ask *why aren't I showing up*… an opaque ranking system creates support load and erodes trust fast… give pros a lever to pull instead of feeling punished by a black box"). New `lib/pro/visibilityHealth.ts` = PURE `evaluateProVisibilityHealth` (every decision, unit-tested) + impure `loadProVisibilityHealth` (11 parallel reads) — a READ over signals ranking ALREADY consumes; instruments nothing new, never changes a score. FIVE levers, each both a real discovery input AND pro-actionable: **`BOOKABLE`** (reuses `checkProReadiness` + `blockerCopy` — the DOMINANT answer, since an unbookable pro is filtered out of discovery entirely, so every other lever is moot; consolidation, not a re-derive), **`AVAILABILITY`** (`ProfessionalAvailabilityStat` → the spec's "booked-out percentage is high" example), **`LOOK_COVERAGE`** (own feed-eligible looks + tag/service breadth → the spec's "add more Looks to widen your tag matches" example; also surfaces REJECTED looks, a real "why" nothing else surfaced), **`BOOKING_CONVERSION`** (`LookPostConversionStat`), **`RELIABILITY`** (`ProfessionalBadgeStat`). **Deliberately NOT surfaced:** `underbooked_pro_boost` (a fairness on-ramp paying MORE lift the FEWER bookings a pro has — publishing it invites gaming, i.e. throttle bookings to farm lift, and insults the pros earning it); `price_fit`/`proximity_fit`/`visual`/`category`/`occasion`/`follow`/`relationship` (all VIEWER-relative — they score a (viewer, look) pair, not the pro, so there's no lever and the per-viewer detail isn't the pro's to see); and any weight/score/formula (direction + lever only, anti-gaming §5.6 — a unit test sweeps every copy branch to enforce it). **Honesty rules (§6.5 exists to buy trust):** (1) `UNKNOWN` is a FIRST-CLASS status — every aggregate here is cron-populated, so an existence probe on `ProfessionalAvailabilityStat` distinguishes "no openings" from "the cron never ran" (without it a fresh deploy tells EVERY pro they have no availability), and a stale row (vs the badge engine's own 6h `LOOK_BADGE_AVAILABILITY_STAT_MAX_AGE_MS`) also reports as unmeasured — never the pro's fault; (2) nothing is invented — §6.5's THIRD example string ("your response time to consult messages is affecting your ranking") has **NO backing capture** (no column, no aggregate; only the cancellation prong of `pro_reliability` ever shipped) and no ranking term reads it, so it ships in `notMeasured` rather than faked (the #638 serve-log-only posture); (3) the booked-out lever states BOTH sides — high `fullness14d` lowers the openness lift AND earns the `BOOKING_OUT` badge, and staying full is a legitimate choice (the spec's own example is one-sided; a pro told only the downside would conclude success is punished). Thresholds REUSE the shipped SSOTs (`LOOK_BADGE_THRESHOLDS.bookingOutMinFullness`/`.availableSoonMaxDays`, `PERSONALIZED_RANK_WEIGHTS.conversionTargetRate`/`.reliabilityFloorRate`) so the dashboard can't drift from what ranking does; only the coverage bars are new display heuristics (documented as such, like `creatorAnalytics`'s `engagementScore`). Surface: a SECTION on `/pro/dashboard` (the gap is ON the dashboard; a new page would be buried — `/pro/dashboard` isn't even in `PRO_HEADER_TABS`) + `GET /api/v1/pro/visibility` sharing the SAME loader (the established `/api/v1/pro/overview` anti-drift rule). All look reads are counts/groupBys/aggregates scoped to the CALLER'S OWN id → `check:tenant-aware-discovery` passes with no tenant helper (#638's shape); tag breadth counted from the TAG side (guard-free + the only single-query distinct count). NO migration/enum/flag/cron/notif; DTO registered → `gen:api-schema`. Pure pro-facing read, no notif surface → merges when green. Tests: 28 pure units + 3 route units + `tests/integration/pro-visibility-health.test.ts` (real-PG: status/moderation bucketing, tag+service breadth over feed-eligible looks only, cross-pro isolation, the availability probe, JSON-safety). **Still deferred:** a `response_time` capture + ranking term (§4.2 lists it as a `pro_reliability` prong; needs a new aggregate off `Message.senderUserId`/`createdAt` — nothing computes it) and iOS deep-linking the fix actions to native screens (tovis-ios BACKLOG `A-VIS1a` — hrefs are web paths and the native equivalents live in other tabs).
  - [ ] **Step 16 — board lifecycle (§7.5):** absent — only the event-proximity decay + one line of static post-event copy; no auto-archive, no "how did it go" prompt, no style-history conversion.
  - [ ] **Step 17 — small-market fallback (§4.7):** absent (no widen-radius → relax-taste → raise-inspiration degradation anywhere).
  - [ ] **Step 18 — cross-board suggestions (§6.4):** absent (lowest priority by design).
  - [ ] **§2.1 similar-user priors:** absent (needs user volume — standing deferral).
  - [ ] **Smaller gaps inside "done" items:** taste graph inputs are likes(1)+board-saves(2) only — comments/shares feed only the global backbone, **bookings strengthen nothing** (spec's strongest signal; remix/booking rates in §4.1 also untracked); re-view/zoom/screenshot not captured (§2); Bridal question set missing "current hair color" + Color asks named color not level (§7.3); board type/answers editable via PATCH API but NO edit UI (only event date), self-profile write-through creation-only (§7.3); board feed emits zero impressions (`BOARD` source reserved, no writer) and sends no seen-ids (§4.4/§4.1); §6.6 hair/skin chips stored-not-scored outside the board-feed tag approximation, no skin-tone chip; `viewer_event_date` now drives ranking + the on-page countdown + §8 milestone countdown NOTIFICATIONS (#621) — event-date badges still pending (§8/§5).
...
- [ ] **Wire the "other pros" rail to its own endpoint — Tori 2026-07-19: "something that got lost along the way, but I do want to implement it."** `GET /api/v1/availability/other-pros` exists, is tested, has a DTO (`AvailabilityOtherProsOk`) and shares the live `loadOtherProsNearbyCached` loader — but **nothing fetches it**, so a consumer scan reads it as an orphan. It is not dead weight: it is the **unwired server half** of a feature whose client half already exists. `useAvailability.ts`'s `loadOtherProsOnly()` (`refreshKind: 'other-pros-only'`) refreshes that one rail by **refetching the entire bootstrap window** with `includeOtherPros: true`. Point it at the dedicated route instead — same cached loader, 120s cache keyed on `scheduleConfigVersion`, and it stops paying for a full-window availability recompute to update a side rail. ⚠️ **Do NOT delete this route in an orphan sweep** (explicitly kept out of the round-3 step-16 cleanup for this reason). 🔴 **Its placement logic has DRIFTED and must be fixed as part of wiring it — do not just point the client at it** (diffed in F9, 2026-07-22): the route forks `validateAvailabilityPlacement`/`resolveAvailabilityPlacement` from `lib/availability/core/placement.ts`, and its copy (a) never passes `professionalTimeZone` and hardcodes `fallbackTimeZone: 'UTC'`, so a location with no timezone resolves to **UTC** instead of the pro's zone, (b) returns a narrower result with no `timeZoneSource`/`workingHours`/`stepMinutes`/`leadTimeMinutes`/`locationBufferMinutes`, and (c) skips the canonical's "requested location exists" check. The two `AvailabilityPlacementResult` types share a name and are different types, so replacing the fork changes the response shape and error mapping — which is why F9 left it rather than half-fixing an unwired route. Prefer calling the exported canonical over patching the fork.
