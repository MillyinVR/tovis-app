...
- [ ] **OPEN — full spec-vs-code audit 2026-07-12** (5-agent code audit; supersedes the thinner "deferred" list that was here — measured against the whole spec, roughly build-order steps 8–18 are open). Grouped by build-order step:
  - [~] **Step 8 — Badge system (§5) + commitment tiers (§5.3) + selection/rotation/TTL (§5.4–5.5, §5.7)** — IN PROGRESS this session (branch `feat/algo-badge-engine`). Ships with the §9 badge-serve logging + holdout scaffolding (spec: build metrics WITH their first consumer). Impression capping (§4.6) NOT included.
