...
- [~] Visual-embedding pgvector pipeline (§6.0) — **built, PR pending merge** (branch `feat/algo-visual-embeddings`). Migration `20260710000000_add_visual_embedding_vectors`: `CREATE EXTENSION vector` + `LookPostEmbedding` (vector(1024) per look) + `ClientTasteVector`/`BoardTasteVector` (§6.1 global/local taste vectors) + `EMBED_LOOK_POST_IMAGE` LooksSocialJob. Embed-at-upload via the publish mutation policy; provider = Voyage AI `voyage-multimodal-3.5` (`VOYAGE_API_KEY`, graceful no-op when unset); `pnpm backfill:look-embeddings` catches up the corpus. Taste vectors = decayed signal-weighted average (same weights/half-life as forYouFeed affinity), refreshed daily by `/api/internal/jobs/taste-vectors`. Local/CI postgres image switched to `imresamu/postgis:16-3.4-bundle0` (postgis+pgvector). **Operator:** provision `VOYAGE_API_KEY` in Vercel, then run the backfill; recreate local dev/test containers on the new image. Follow-ups: ranking consumption (cosine `visual_similarity` in forYouRanking + §4.4 board feed), corpus-wide ANN (hnsw index ships with that consumer), look-side attribute schema for representation/feasibility scoring (deferred from §6.6, unblocked by neither tags nor embeddings alone).
