...
function writeBaseline(violations) {
  ...
  const header = [
    ...
    '#',
    '# 🔴 A black SHADOW has NO replacement, and an earlier version of this',
    '# header prescribed `rgb(var(--shadow-color) / a)`. That swap DELETES the',
    '# shadow in dark mode: --shadow-color is 10 20 19, the dark page ground',
    '# itself, so an ink shadow has nothing left to darken. Measured on isolated',
    '# probes, a 0.78-alpha shadow drops from 12.0 units of separation from the',
    '# page to 1.7. The `shadow-[…rgb(0 0 0/a)]` entries and the inline',
    "# `boxShadow: '… rgba(0,0,0,a)'` ones are ONE family, and it is BLOCKED on a",
    '# per-mode shadowColor decision. Leave every one of them alone.',
    '#',
    ...
    '#      in MediaUploader / ImageEditModal / NewMediaPostForm, and the Leaflet',
    "#      markers in search/MapView, which sit on OpenStreetMap's raster tiles.",
