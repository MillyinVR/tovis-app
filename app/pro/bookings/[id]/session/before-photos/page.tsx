...
export default async function ProBeforePhotosPage(props: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main style={{ maxWidth: 960, margin: '24px auto 90px', padding: '0 16px', fontFamily: 'system-ui' }}>
      <a href={`/pro/bookings/${encodeURIComponent(bookingId)}/session`} style={{ fontSize: 12, color: '#555', textDecoration: 'none' }}>
        ← Back to session
      </a>
      ...
      <section style={{ marginTop: 16 }}>
        ...
        {items.length === 0 ? (
          ...
          <div style={{ marginTop: 10, display: 'grid', gap: 10 }}>
            {items.map((m) => (
              <div key={m.id} style={{ border: '1px solid #eee', borderRadius: 12, background: '#fff', padding: 12, display: 'grid', gap: 6 }}>
