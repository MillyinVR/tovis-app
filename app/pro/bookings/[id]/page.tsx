...
function pickBookingLocationTimeZone(value: unknown): string | null {
  if (!isRecord(value)) return null
  return pickString(value.locationTimeZone)
}

function pickAftercareNotes(value: unknown): string | null {
  if (!isRecord(value)) return null
  const aftercareSummary = value.aftercareSummary
  if (!isRecord(aftercareSummary)) return null
  return pickString(aftercareSummary.notes)
}
...
export default async function ProBookingDetailPage(props: {
  ...
  const apptTz = resolveAppointmentTimeZone(
    pickBookingLocationTimeZone(booking),
    scheduleTz,
  )
  ...
  const aftercareNotes = pickAftercareNotes(booking)
  ...
  return (
    <main className="mx-auto w-full max-w-240 px-4 pb-24 pt-8 text-textPrimary">
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div className="min-w-0">
            ...
            <div className="mt-2 text-[12px] font-semibold text-textSecondary">
              ...
              {dur ? ` • ${dur} min` : ''}
              {showTzBadge ? (
                <span className="text-textSecondary/70"> · {apptTz}</span>
              ) : null}
            </div>
          </div>
          ...
          <div className="shrink-0 md:text-right">
            ...
            <div className="text-[18px] font-black text-textPrimary">
              ${total}
            </div>
      ...
      <div className="grid gap-6">
        <SectionCard
          title="Aftercare"
          subtitle="Snapshot saved on the booking (if provided)."
        >
          {aftercareNotes ? (
        ...
        <SectionCard
          title="Timing"
          subtitle="State timestamps for this booking."
        >
