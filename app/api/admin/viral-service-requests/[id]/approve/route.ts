...
export async function POST(req: NextRequest, ctx: Ctx) {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      const request = await updateViralRequestStatus(tx, {
        requestId,
      ...
      const notifications = await enqueueViralRequestApprovalNotifications(tx, {
        requestId,
      })
    ...
    return jsonOk({
      ...
      notifications: toViralRequestApprovalNotificationsDto(result.notifications),
    })
  } catch (error: unknown) {
    console.error('POST /api/admin/viral-service-requests/[id]/approve error', error)
