// app/api/internal/jobs/saved-look-activation/route.ts
//
// Cron: 20 10 * * * (daily; see vercel.json)
//
// §6.8 saved-not-booked activation, gated by the §8.1 re-engagement notification
// budget. Scans aging saves whose pro now has a near-term calendar opening
// (ProfessionalAvailabilityStat, #604), excludes pairs the client already booked
// or was recently nudged, and sends at most one gentle "your saved pro has an
// opening" nudge per client per run — pooled under the weekly re-engagement cap
// (lib/notifications/reEngagementBudget.ts). Daily (not hourly) because
// re-engagement nudges should be calm; delivery-time quiet hours still apply.
//
...
async function runJob(req: Request) {
  ...
  try {
    const summary = await runSavedLookActivation(prisma, { now: new Date() })
    ...
    console.error('GET /api/internal/jobs/saved-look-activation error', {
