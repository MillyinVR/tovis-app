// app/api/looks/[id]/report/route.test.ts
...
describe('app/api/looks/[id]/report/route.ts', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('POST reports by canonical lookPostId and returns accepted on first submit', async () => {
    const res = await POST(
      new Request('http://localhost/api/looks/look_1/report', {
        method: 'POST',
      }),
      makeCtx('look_1'),
    )
  ...
  it('returns 400 when the route param is blank', async () => {
    const res = await POST(
      new Request('http://localhost/api/looks/%20%20/report', {
        method: 'POST',
      }),
      makeCtx('   '),
    )
  ...
  it('returns 404 when the canonical lookPostId cannot be resolved', async () => {
    ...
    const res = await POST(
      new Request('http://localhost/api/looks/look_missing/report', {
        method: 'POST',
      }),
  ...
  it('does not treat legacy media ids as fallback identifiers', async () => {
    ...
    const res = await POST(
      new Request('http://localhost/api/looks/media_legacy_1/report', {
        method: 'POST',
      }),
