...
export type GoogleAddressGeocode = {
  placeId: string | null
  formattedAddress: string | null
  lat: number | null
  lng: number | null
...
/**
 * Forward-geocode a full free-text street address (not just a postal code).
 *
 * Used to canonicalize a manually-typed client service address into a
 * formatted address + coordinates + placeId on save, so it is bookable for
 * mobile without the client having to pick an autocomplete suggestion.
 * Throws on transport/Google error or when no result is found.
 */
export async function googleGeocodeAddress(
  address: string,
  countryCode?: string | null,
): Promise<GoogleAddressGeocode> {
  const cc = (countryCode || 'US').trim().toUpperCase().replace(/[^A-Z]/g, '')
  const url = new URL('https://maps.googleapis.com/maps/api/geocode/json')
