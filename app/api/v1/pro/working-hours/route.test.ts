// app/api/v1/pro/working-hours/route.test.ts
...
describe('app/api/v1/pro/working-hours/route.ts', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  describe('GET', () => {
    it('returns defaults when no matching bookable location exists', async () => {
      ...
      const res = await GET(
        makeRequest('GET', '/api/v1/pro/working-hours?locationType=SALON'),
      )
    ...
    it('returns normalized stored workingHours when valid', async () => {
      ...
      const res = await GET(
        makeRequest('GET', '/api/v1/pro/working-hours?locationType=MOBILE'),
      )
  ...
  describe('GET with locationId', () => {
    it('returns the hours for that specific owned bookable location', async () => {
      ...
      const res = await GET(
        makeRequest('GET', '/api/v1/pro/working-hours?locationId=loc_suite_2'),
      )
    ...
    it('returns 404 when the location is not owned or not bookable', async () => {
      ...
      const res = await GET(
        makeRequest('GET', '/api/v1/pro/working-hours?locationId=loc_other'),
      )
  ...
  describe('POST with locationId', () => {
    ...
    it('returns 404 when the targeted location is not owned or not bookable', async () => {
      ...
      const res = await POST(
        makeRequest('POST', '/api/v1/pro/working-hours?locationId=loc_other', {
          workingHours: defaultWorkingHours(),
        }),
      )
    ...
    it('rejects a locationType that does not match the targeted location', async () => {
      ...
      const res = await POST(
        makeRequest(
          'POST',
          '/api/v1/pro/working-hours?locationType=SALON&locationId=loc_mobile_2',
          { workingHours: defaultWorkingHours() },
        ),
      )
  ...
  describe('POST', () => {
    it('rejects missing or invalid locationType', async () => {
      const res = await POST(
        makeRequest('POST', '/api/v1/pro/working-hours', {
          workingHours: defaultWorkingHours(),
        }),
      )
    ...
    it('passes through rate limit response unchanged', async () => {
      ...
      const res = await POST(
        makeRequest('POST', '/api/v1/pro/working-hours?locationType=SALON', {
          workingHours: defaultWorkingHours(),
        }),
      )
    ...
    it('rejects invalid request body', async () => {
      const req = new Request(
        'http://localhost/api/v1/pro/working-hours?locationType=SALON',
        {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
    ...
    it('returns 409 when no matching bookable location exists and does not auto-create one', async () => {
      ...
      const res = await POST(
        makeRequest('POST', '/api/v1/pro/working-hours?locationType=MOBILE', {
          workingHours: defaultWorkingHours(),
        }),
      )
