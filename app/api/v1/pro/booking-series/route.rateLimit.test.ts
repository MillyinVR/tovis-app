...
vi.mock('@/lib/booking/series/flag', () => ({
  recurringAppointmentsEnabled: () => true,
}))

import { POST } from './route'

describe('POST /api/v1/pro/booking-series — pro:bookings:write ceiling', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  function makeRequest(): Request {
    return new Request('http://localhost/api/v1/pro/booking-series', {
      method: 'POST',
  ...
  it('keys the ceiling per professional, not per IP alone', async () => {
    ...
    await POST(makeRequest()).catch(() => undefined)
