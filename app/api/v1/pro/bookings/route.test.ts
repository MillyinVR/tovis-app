...
const IDEMPOTENCY_ROUTE = 'POST /api/v1/pro/bookings'
...
function makeRequest(
  ...
  return new Request('http://localhost/api/v1/pro/bookings', {
    method: 'POST',
    headers: {
...
describe('POST /api/v1/pro/bookings', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('maps booking errors to jsonFail and marks idempotency failed', async () => {
    ...
    it('logs unexpected errors through safe logging helpers and captures the exception', async () => {
      ...
      expect(mocks.safeLogMeta).toHaveBeenCalledWith({
        route: 'POST /api/v1/pro/bookings',
        idempotencyRecordId: 'idem_record_1',
      })

      expect(consoleErrorSpy).toHaveBeenCalledWith(
        'POST /api/v1/pro/bookings error',
        {
          error: {
            name: 'Error',
    ...
    it('logs idempotency failure-update errors through safe logging helpers', async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        'POST /api/v1/pro/bookings idempotency failure update error',
        {
          error: {
            name: 'Error',
