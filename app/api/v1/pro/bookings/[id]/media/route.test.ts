...
const IDEMPOTENCY_ROUTE = 'POST /api/v1/pro/bookings/[id]/media'
...
function makeGetRequest(
  path = 'http://localhost/api/v1/pro/bookings/booking_1/media',
): Request {
...
describe('app/api/v1/pro/bookings/[id]/media/route.ts', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('GET rejects invalid phase query param', async () => {
    const result = await GET(
      makeGetRequest(
        'http://localhost/api/v1/pro/bookings/booking_1/media?phase=BANANA',
      ),
      makeCtx(),
    )
  ...
  it('GET returns internal error, logs safely, and captures exception for unexpected errors', async () => {
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      'GET /api/v1/pro/bookings/[id]/media error',
      {
        error: {
          name: 'Error',
          message:
  ...
  it('POST returns internal error, logs safely, captures exception, and marks idempotency failed for unexpected errors', async () => {
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      'POST /api/v1/pro/bookings/[id]/media error',
      {
        error: {
          name: 'Error',
          message:
