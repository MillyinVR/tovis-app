...
const IDEMPOTENCY_ROUTE = 'POST /api/v1/pro/bookings/[id]/aftercare'
const OPERATION = 'POST /api/v1/pro/bookings/[id]/aftercare'
...
function makeRequest(args?: {
  ...
  return new Request('http://localhost/api/v1/pro/bookings/booking_1/aftercare', {
    method: 'POST',
    headers: {
      'content-type': 'application/json',
      ...(args?.headers ?? {}),
...
describe('app/api/v1/pro/bookings/[id]/aftercare/route.ts', () => {
  ...
  it('POST returns 500 for unexpected errors, logs safely, captures exception, and marks idempotency failed', async () => {
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      'POST /api/v1/pro/bookings/[id]/aftercare error',
      {
        error: {
          name: 'Error',
          message:
