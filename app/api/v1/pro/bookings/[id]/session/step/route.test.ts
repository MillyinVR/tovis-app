// app/api/v1/pro/bookings/[id]/session/step/route.test.ts

import { beforeEach, describe, expect, it, vi } from 'vitest'
import { BookingStatus, Role, SessionStep } from '@prisma/client'
import { bookingError } from '@/lib/booking/errors'

const IDEMPOTENCY_ROUTE = 'POST /api/v1/pro/bookings/[id]/session/step'
...
function makeRequest(
  ...
  return new Request(
    'http://localhost/api/v1/pro/bookings/booking_1/session/step',
    {
      method: 'POST',
      headers: {
...
describe('POST /api/v1/pro/bookings/[id]/session/step', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('returns internal error, logs safely, captures exception, and marks idempotency failed for unexpected errors', async () => {
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      'POST /api/v1/pro/bookings/[id]/session/step error',
      {
        requestId: null,
        error: {
          name: 'Error',
