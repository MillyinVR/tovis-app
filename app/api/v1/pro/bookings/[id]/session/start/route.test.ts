// app/api/v1/pro/bookings/[id]/start/route.test.ts

import { beforeEach, describe, expect, it, vi } from 'vitest'
import { BookingStatus, Role, SessionStep } from '@prisma/client'
import { bookingError } from '@/lib/booking/errors'

const IDEMPOTENCY_ROUTE = 'POST /api/v1/pro/bookings/[id]/start'
...
vi.mock('@/lib/idempotency', () => ({
  IDEMPOTENCY_ROUTES: {
    BOOKING_START_SESSION: 'POST /api/v1/pro/bookings/[id]/start',
  },
}))
...
function makeRequest(headers?: Record<string, string>): Request {
  return new Request('http://localhost/api/v1/pro/bookings/booking_1/start', {
    method: 'POST',
    headers: {
...
describe('POST /api/v1/pro/bookings/[id]/start', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('returns internal error, logs safely, and marks idempotency failed for unexpected errors', async () => {
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      'POST /api/v1/pro/bookings/[id]/start error',
      {
        requestId: null,
        error: {
          name: 'Error',
  ...
  it('passes explicitSelection and actorUserId when body contains explicitSelection: true', async () => {
    ...
    const req = new Request(
      'http://localhost/api/v1/pro/bookings/booking_1/start',
      {
        method: 'POST',
        headers: {
          'content-type': 'application/json',
