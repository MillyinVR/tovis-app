...
function makeRequest(
  ...
  return new Request('http://localhost/api/v1/pro/bookings/booking_1/rebook', {
    method: 'POST',
    headers,
...
function expectIdempotencyStarted(key = 'idem_rebook_1'): void {
...
describe('POST /api/v1/pro/bookings/[id]/rebook', () => {
  beforeEach(() => {
    vi.useFakeTimers()
  ...
  it('returns handled idempotency response when idempotency key is missing before mutating', async () => {
  ...
  it('returns handled idempotency response when idempotency key is reused with a different request', async () => {
    const handledResponse = {
      ok: false,
      status: 409,
...
it('creates a rebooked booking in BOOK mode and completes idempotency', async () => {
  ...
  it('maps BookingError through bookingJsonFail and marks idempotency failed', async () => {
    ...
    expect(mocks.failStartedRouteIdempotency).toHaveBeenCalledWith({
      idempotencyRecordId: 'idem_record_1',
      operation: 'POST /api/v1/pro/bookings/[id]/rebook',
    })
  ...
  it('logs unexpected errors through safe logging helpers and marks idempotency failed', async () => {
    ...
    try {
      ...
      expect(mocks.safeLogMeta).toHaveBeenCalledWith({
        route: 'POST /api/v1/pro/bookings/[id]/rebook',
        idempotencyRecordId: 'idem_record_1',
      })

      expect(consoleErrorSpy).toHaveBeenCalledWith(
        'POST /api/v1/pro/bookings/[id]/rebook error',
        {
          error: {
            name: 'Error',
            message: 'boom',
