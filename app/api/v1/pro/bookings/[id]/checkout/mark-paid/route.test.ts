...
function makeRequest(
  ...
  return new Request(
    'http://localhost/api/v1/pro/bookings/booking_1/checkout/mark-paid',
    {
      method: 'POST',
      headers: {
        'content-type': 'application/json',
...
describe('POST /api/v1/pro/bookings/[id]/checkout/mark-paid', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('maps BookingError and fails started idempotency', async () => {
    ...
    expect(mocks.failStartedRouteIdempotency).toHaveBeenCalledWith({
      idempotencyRecordId: 'idem_record_1',
      operation: 'POST /api/v1/pro/bookings/[id]/checkout/mark-paid',
    })
  ...
  it('returns INTERNAL_ERROR for unexpected failures, logs safely, and fails started idempotency', async () => {
    ...
    expect(mocks.safeLogMeta).toHaveBeenCalledWith({
      route: 'POST /api/v1/pro/bookings/[id]/checkout/mark-paid',
      idempotencyRecordId: 'idem_record_1',
    })

    expect(consoleErrorSpy).toHaveBeenCalledWith(
      'POST /api/v1/pro/bookings/[id]/checkout/mark-paid error',
      {
        error: {
          name: 'Error',
          message: 'boom',
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      'POST /api/v1/pro/bookings/[id]/checkout/mark-paid idempotency failure update error',
      {
        error: {
          name: 'Error',
