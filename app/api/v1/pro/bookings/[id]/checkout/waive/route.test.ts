...
function makeRequest(
  ...
  return new Request(
    'http://localhost/api/v1/pro/bookings/booking_1/checkout/waive',
    {
      method: 'POST',
      headers: {
        'content-type': 'application/json',
...
describe('POST /api/v1/pro/bookings/[id]/checkout/waive', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('maps BookingError and fails started idempotency', async () => {
    ...
    expect(mocks.failStartedRouteIdempotency).toHaveBeenCalledWith({
      idempotencyRecordId: 'idem_record_1',
      operation: 'POST /api/v1/pro/bookings/[id]/checkout/waive',
    })
  ...
  it('returns INTERNAL_ERROR for unexpected failures, logs safely, and fails started idempotency', async () => {
    ...
    expect(mocks.safeLogMeta).toHaveBeenCalledWith({
      route: 'POST /api/v1/pro/bookings/[id]/checkout/waive',
      idempotencyRecordId: 'idem_record_1',
    })

    expect(consoleErrorSpy).toHaveBeenCalledWith(
      'POST /api/v1/pro/bookings/[id]/checkout/waive error',
      {
        error: {
          name: 'Error',
          message: 'boom',
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      'POST /api/v1/pro/bookings/[id]/checkout/waive idempotency failure update error',
      {
        error: {
          name: 'Error',
