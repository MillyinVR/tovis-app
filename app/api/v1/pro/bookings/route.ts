// app/api/v1/pro/bookings/route.ts
import {
  Prisma,
  Role,
  type BookingStatus,
  ...
  try {
    ...
    if (idempotencyRecordId) {
      await failIdempotency({ idempotencyRecordId }).catch((failError: unknown) => {
        console.error('POST /api/v1/pro/bookings idempotency failure update error', {
    ...
    captureBookingException({ error, route: 'POST /api/v1/pro/bookings' })
