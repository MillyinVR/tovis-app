...
import { GET } from '@/app/api/v1/pro/notifications/route'

describe('GET /api/v1/pro/notifications', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it('returns auth response when requirePro fails', async () => {
    ...
    const req = new Request('http://localhost/api/v1/pro/notifications')
  ...
  it('passes cursor, unread filter, and eventKey filter through', async () => {
    ...
    const req = new Request(
      'http://localhost/api/v1/pro/notifications?take=25&cursor=notif_cursor&unread=1&eventKey=REVIEW_RECEIVED',
    )
  ...
  it('clamps take to the maximum of 100', async () => {
    ...
    const req = new Request(
      'http://localhost/api/v1/pro/notifications?take=999',
    )
  ...
  it('clamps take to the minimum of 1', async () => {
    ...
    const req = new Request(
      'http://localhost/api/v1/pro/notifications?take=0',
    )
  ...
  it('uses fallback take when take is invalid', async () => {
    ...
    const req = new Request(
      'http://localhost/api/v1/pro/notifications?take=not-a-number',
    )
  ...
  it('returns 400 when eventKey is invalid', async () => {
    ...
    const req = new Request(
      'http://localhost/api/v1/pro/notifications?eventKey=totally_wrong',
    )
  ...
  it('accepts representative supported notification event keys', async () => {
    ...
    for (const eventKey of supportedEventKeys) {
      await GET(
        new Request(
          `http://localhost/api/v1/pro/notifications?eventKey=${eventKey}`,
        ),
      )
    }
  ...
  it('treats unread=true and unread=yes as unreadOnly', async () => {
    ...
    await GET(
      new Request('http://localhost/api/v1/pro/notifications?unread=true'),
    )
    await GET(
      new Request('http://localhost/api/v1/pro/notifications?unread=yes'),
    )
