...
export async function GET(_request: Request, ctx: RouteContext) {
  try {
    ...
    console.error('GET /api/v1/pro/offerings/[id] error', error)
  ...
  try {
    ...
    const result: UpdateResult = await prisma.$transaction(async (tx) => {
      ...
      if (
        ...
        offersInSalonIn === undefined
      ) {
        return {
          kind: 'ERROR',
          status: 400,
          msg: 'offersInSalon must be boolean.',
        }
      }

      if (
        Object.prototype.hasOwnProperty.call(body, 'offersMobile') &&
        offersMobileIn === undefined
      ) {
        return {
          kind: 'ERROR',
          status: 400,
          msg: 'offersMobile must be boolean.',
        }
      }
      ...
      const nextOffersInSalon =
        typeof offersInSalonIn === 'boolean'
          ? offersInSalonIn
          : Boolean(existing.offersInSalon)

      const nextOffersMobile =
        typeof offersMobileIn === 'boolean'
          ? offersMobileIn
          : Boolean(existing.offersMobile)

      const serviceOk = Boolean(
        existing.service.isActive && existing.service.category?.isActive,
      )

      if (!serviceOk) {
        const isActiveIn = Object.prototype.hasOwnProperty.call(
          body,
          'isActive',
        )
      ...
      if (!nextOffersInSalon && !nextOffersMobile) {
        return {
          kind: 'ERROR',
          status: 400,
          msg: 'Enable at least Salon or Mobile.',
        }
      }

      const ensureSalon =
        offersInSalonIn === true && existing.offersInSalon === false

      const ensureMobile =
        offersMobileIn === true && existing.offersMobile === false
      ...
      if (
        ...
        return {
          kind: 'ERROR',
          status: 400,
          msg: 'description must be string or null.',
        }
      }
      ...
      if (
        ...
        return {
          kind: 'ERROR',
          status: 400,
          msg: 'customImageUrl must be string or null.',
        }
      }
      ...
      const salonPriceIn = Object.prototype.hasOwnProperty.call(
        body,
      ...
      const salonDurationIn = Object.prototype.hasOwnProperty.call(
        body,
      ...
      const mobilePriceIn = Object.prototype.hasOwnProperty.call(
        body,
      ...
      const mobileDurationIn = Object.prototype.hasOwnProperty.call(
        body,
      ...
      if (
        ...
        return {
          kind: 'ERROR',
          status: 400,
          msg: 'salonPriceStartingAt must be string or null.',
        }
      }

      if (
        Object.prototype.hasOwnProperty.call(body, 'salonDurationMinutes') &&
        salonDurationIn === undefined
      ) {
        return {
          kind: 'ERROR',
          status: 400,
          msg: 'Invalid salonDurationMinutes.',
        }
      }

      if (
        Object.prototype.hasOwnProperty.call(body, 'mobilePriceStartingAt') &&
        mobilePriceIn === undefined
      ) {
        return {
          kind: 'ERROR',
          status: 400,
          msg: 'mobilePriceStartingAt must be string or null.',
        }
      }

      if (
        Object.prototype.hasOwnProperty.call(body, 'mobileDurationMinutes') &&
        mobileDurationIn === undefined
      ) {
        return {
          kind: 'ERROR',
          status: 400,
      ...
      if (!nextOffersInSalon) {
        ...
        if (salonPriceIn !== undefined) {
          ...
          try {
            patchSalonPrice = parsePriceOrThrow(
              salonPriceIn,
              existing.service.minPrice,
              'Salon',
            )
            data.salonPriceStartingAt = patchSalonPrice
          } catch (e: unknown) {
            const msg =
              e instanceof Error ? e.message : 'Invalid salon price.'
            ...
            return {
              ...
              msg,
              extra: {
                minPrice: moneyToString(existing.service.minPrice) ?? '0.00',
              },
            }
          }
        }
        ...
        const finalSalonDuration =
          patchSalonDuration ?? existing.salonDurationMinutes

        const finalSalonPrice =
          patchSalonPrice ?? existing.salonPriceStartingAt
      ...
      if (!nextOffersMobile) {
        ...
        if (mobilePriceIn !== undefined) {
          ...
          try {
            patchMobilePrice = parsePriceOrThrow(
              mobilePriceIn,
              existing.service.minPrice,
              'Mobile',
            )
            ...
            const msg =
              e instanceof Error ? e.message : 'Invalid mobile price.'
        ...
        const finalMobileDuration =
          patchMobileDuration ?? existing.mobileDurationMinutes

        const finalMobilePrice =
          patchMobilePrice ?? existing.mobilePriceStartingAt
    ...
    console.error('PATCH /api/v1/pro/offerings/[id] error', error)
    return jsonFail(500, 'Internal server error.')
  }
}
...
export async function DELETE(_request: Request, ctx: RouteContext) {
  try {
    ...
    console.error('DELETE /api/v1/pro/offerings/[id] error', error)
    return jsonFail(500, 'Internal server error.')
  }
}