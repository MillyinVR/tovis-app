// app/api/v1/pro/looks/[id]/route.test.ts
...
function makePatchRequest(body: unknown): Request {
  return new Request('http://localhost/api/v1/pro/looks/look_1', {
    method: 'PATCH',
    headers: {
...
describe('app/api/v1/pro/looks/[id]/route.ts', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('passes through failed pro auth responses unchanged for GET', async () => {
    ...
    const res = await GET(
      new Request('http://localhost/api/v1/pro/looks/look_1'),
      makeCtx('look_1'),
    )
  ...
  it('returns 400 for GET when route id is missing', async () => {
    const res = await GET(
      new Request('http://localhost/api/v1/pro/looks/%20'),
      makeCtx('   '),
    )
  ...
  it('delegates GET to the publication service and awaits promised params', async () => {
    ...
    const req = new Request('http://localhost/api/v1/pro/looks/look_2')
  ...
  it('returns 404 for GET when the look post is not found', async () => {
    ...
    const res = await GET(
      new Request('http://localhost/api/v1/pro/looks/look_missing'),
