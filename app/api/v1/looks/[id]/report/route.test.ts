// app/api/v1/looks/[id]/report/route.test.ts
...
const mocks = vi.hoisted(() => {
  ...
  const createLookPostReport = vi.fn()
...
vi.mock('@/lib/looks/reporting', () => ({
  createLookPostReport: mocks.createLookPostReport,
}))
...
describe('app/api/v1/looks/[id]/report/route.ts', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    ...
    mocks.createLookPostReport.mockResolvedValue({
      status: 'accepted',
    })
  })

  it('POST reports by canonical lookPostId and returns accepted on first submit', async () => {
    const res = await POST(
      new Request('http://localhost/api/v1/looks/look_1/report', {
        method: 'POST',
      }),
      makeCtx('look_1'),
    )
    ...
    expect(mocks.createLookPostReport).toHaveBeenCalledWith(mocks.prisma, {
      lookPostId: 'look_1',
      userId: 'user_1',
    })
  ...
  it('returns 400 when the route param is blank', async () => {
    const res = await POST(
      new Request('http://localhost/api/v1/looks/%20%20/report', {
        method: 'POST',
      }),
      makeCtx('   '),
    )
    ...
    expect(mocks.createLookPostReport).not.toHaveBeenCalled()
  })
  ...
  it('returns 404 when the canonical lookPostId cannot be resolved', async () => {
    ...
    const res = await POST(
      new Request('http://localhost/api/v1/looks/look_missing/report', {
        method: 'POST',
      }),
  ...
  it('does not treat legacy media ids as fallback identifiers', async () => {
    ...
    const res = await POST(
      new Request('http://localhost/api/v1/looks/media_legacy_1/report', {
        method: 'POST',
      }),
  ...
  it('returns 500 when report creation fails', async () => {
    mocks.createLookPostReport.mockRejectedValue(
