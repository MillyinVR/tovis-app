// app/api/v1/looks/[id]/comments/[commentId]/report/route.test.ts
...
describe('app/api/v1/looks/[id]/comments/[commentId]/report/route.ts', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('POST reports a comment by canonical lookPostId + commentId and returns accepted on first submit', async () => {
    const res = await POST(
      new Request('http://localhost/api/v1/looks/look_1/comments/comment_1/report', {
        method: 'POST',
      }),
  ...
  it('returns 400 when the look route param is blank', async () => {
    const res = await POST(
      new Request('http://localhost/api/v1/looks/%20%20/comments/comment_1/report', {
        method: 'POST',
      }),
      makeCtx('   ', 'comment_1'),
    )
  ...
  it('returns 400 when the comment route param is blank', async () => {
    const res = await POST(
      new Request('http://localhost/api/v1/looks/look_1/comments/%20%20/report', {
        method: 'POST',
      }),
      makeCtx('look_1', '   '),
    )
  ...
  it('returns 404 when the canonical lookPostId cannot be resolved', async () => {
    ...
    const res = await POST(
      new Request(
        'http://localhost/api/v1/looks/look_missing/comments/comment_1/report',
        {
          method: 'POST',
        },
      ),
  ...
  it('returns 404 when the comment cannot be resolved under the canonical lookPostId', async () => {
    ...
    const res = await POST(
      new Request(
        'http://localhost/api/v1/looks/look_1/comments/comment_missing/report',
        {
          method: 'POST',
        },
      ),
  ...
  it('does not treat legacy media ids as fallback identifiers', async () => {
    ...
    const res = await POST(
      new Request(
        'http://localhost/api/v1/looks/media_legacy_1/comments/comment_1/report',
        {
          method: 'POST',
        },
      ),
