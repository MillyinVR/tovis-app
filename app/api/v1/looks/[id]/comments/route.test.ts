// app/api/v1/looks/[id]/comments/route.test.ts
...
describe('app/api/v1/looks/[id]/comments/route.ts', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('GET lists approved comments by canonical lookPostId and returns the canonical id in the contract', async () => {
    ...
    const res = await GET(
      new Request('http://localhost/api/v1/looks/look_1/comments?limit=5'),
      makeCtx('look_1'),
    )
  ...
  it('returns 404 when the canonical lookPostId cannot be resolved', async () => {
    ...
    const res = await GET(
      new Request('http://localhost/api/v1/looks/look_missing/comments'),
  ...
  it('does not treat legacy media ids as fallback identifiers', async () => {
    ...
    const res = await GET(
      new Request('http://localhost/api/v1/looks/media_legacy_1/comments'),
