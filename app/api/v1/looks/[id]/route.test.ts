// app/api/v1/looks/[id]/route.test.ts
...
describe('app/api/v1/looks/[id]/route.ts', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('fetches detail by canonical lookPost.id for a public viewer', async () => {
    ...
    const res = await GET(
      new Request('http://localhost/api/v1/looks/look_1'),
      makeCtx('look_1'),
    )
  ...
  it('returns 404 when the look is not found', async () => {
    ...
    const res = await GET(
      new Request('http://localhost/api/v1/looks/look_missing'),
  ...
  it('does not support legacy media ids as a fallback lookup', async () => {
    ...
    const res = await GET(
      new Request('http://localhost/api/v1/looks/media_legacy_1'),
  ...
  it('returns 400 when the route param is blank', async () => {
    const res = await GET(
      new Request('http://localhost/api/v1/looks/%20%20'),
      makeCtx('   '),
    )
    const body = await readJson(res)
