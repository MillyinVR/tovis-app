// app/api/v1/me/following/route.test.ts
...
describe('app/api/v1/me/following/route.ts', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('returns the current viewer following list', async () => {
    const res = await GET(
      new Request('http://localhost/api/v1/me/following'),
    )
  ...
  it('passes parsed take and skip values into the shared list helper', async () => {
    ...
    const res = await GET(
      new Request('http://localhost/api/v1/me/following?take=2&skip=6'),
    )
