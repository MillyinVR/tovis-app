// app/api/v1/client/follow/[handle]/route.ts
//
...
export async function GET(_req: Request, ctx: RouteContext<{ handle: string }>) {
  try {
    ...
    console.error('GET /api/v1/client/follow/[handle] error', error)
  ...
  try {
    ...
    if (state.following) {
      ...
      await createClientFollowNotification({
        ...
        console.error(
          'POST /api/v1/client/follow/[handle] notify error',
          error,
        )
      })
    ...
    console.error('POST /api/v1/client/follow/[handle] error', error)
