// app/api/v1/client/bookings/[id]/review/route.test.ts
...
const IDEMPOTENCY_ROUTE = 'POST /api/v1/client/bookings/[id]/review'
const ROUTE_OPERATION = 'POST /api/v1/client/bookings/[id]/review'
...
function makeRequest(body: unknown, headers?: Record<string, string>) {
  return new NextRequest(
    'http://localhost/api/v1/client/bookings/booking_1/review',
    {
      method: 'POST',
      headers: {
...
describe('app/api/v1/client/bookings/[id]/review/route.ts POST', () => {
  beforeEach(() => {
    vi.resetAllMocks()
  ...
  it('creates a review, attaches pro media, creates client media, writes closeout audit, notifies pro, and completes idempotency', async () => {
    ...
    expect(mocks.createBookingCloseoutAuditLog).toHaveBeenCalledWith({
      ...
      route: 'app/api/v1/client/bookings/[id]/review/route.ts:POST',
      requestId: 'req_review_1',
      idempotencyKey: 'idem_review_1',
      oldValue: null,
      newValue: {
  ...
  it('logs notification failure but still returns success after completing idempotency', async () => {
    ...
    try {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        'POST /api/v1/client/bookings/[id]/review pro notification error',
        expect.any(Error),
      )
    } finally {
