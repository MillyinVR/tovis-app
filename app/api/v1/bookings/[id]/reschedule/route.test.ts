// app/api/v1/bookings/[id]/reschedule/route.test.ts
...
function makeRequest(body: unknown, headers?: HeadersInit): Request {
  return new Request('http://localhost/api/v1/bookings/booking_1/reschedule', {
    method: 'POST',
    headers: {
  ...
  it('logs unexpected errors through safe logging helpers, fails idempotency, and returns INTERNAL_ERROR', async () => {
    ...
    try {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        'POST /api/v1/bookings/[id]/reschedule error',
        {
          error: {
            name: 'Error',
            message: 'boom',
