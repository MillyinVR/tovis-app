// app/api/v1/pros/[id]/follow/route.test.ts
...
describe('app/api/v1/pros/[id]/follow/route.ts', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('GET returns stable follow state by canonical professionalId', async () => {
    const res = await GET(
      new Request('http://localhost/api/v1/pros/pro_1/follow'),
      makeCtx('pro_1'),
    )
  ...
  it('returns 400 when the route param is blank', async () => {
    const res = await GET(
      new Request('http://localhost/api/v1/pros/%20%20/follow'),
      makeCtx('   '),
    )
    const body = await readJson(res)
  ...
  it('GET returns 404 when the canonical professionalId cannot be resolved', async () => {
    ...
    const res = await GET(
      new Request('http://localhost/api/v1/pros/pro_missing/follow'),
      makeCtx('pro_missing'),
    )
    const body = await readJson(res)
  ...
  it('does not treat other identifiers as fallback professional ids', async () => {
    ...
    const res = await GET(
      new Request('http://localhost/api/v1/pros/user_legacy_1/follow'),
