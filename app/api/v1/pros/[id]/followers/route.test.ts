// app/api/v1/pros/[id]/followers/route.test.ts
...
describe('app/api/v1/pros/[id]/followers/route.ts', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('returns the followers list by canonical professionalId', async () => {
    const res = await GET(
      new Request('http://localhost/api/v1/pros/pro_1/followers'),
      makeCtx('pro_1'),
    )
  ...
  it('passes parsed take and skip values into the shared list helper', async () => {
    ...
    const res = await GET(
      new Request('http://localhost/api/v1/pros/pro_1/followers?take=2&skip=6'),
      makeCtx('pro_1'),
    )
  ...
  it('returns 400 when the route param is blank', async () => {
    const res = await GET(
      new Request('http://localhost/api/v1/pros/%20%20/followers'),
      makeCtx('   '),
    )
    const body = await readJson(res)
  ...
  it('returns 404 when the canonical professionalId cannot be resolved', async () => {
    ...
    const res = await GET(
      new Request('http://localhost/api/v1/pros/pro_missing/followers'),
      makeCtx('pro_missing'),
    )
    const body = await readJson(res)
  ...
  it('does not treat other ids as fallback professional ids', async () => {
    ...
    const res = await GET(
      new Request('http://localhost/api/v1/pros/user_legacy_1/followers'),
