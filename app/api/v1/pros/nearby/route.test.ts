// app/api/v1/pros/nearby/route.test.ts
...
describe('app/api/v1/pros/nearby/route.ts', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('returns 400 when lat is missing', async () => {
    const res = await GET(makeRequest('/api/v1/pros/nearby?lng=-117.1611'))
  ...
  it('rejects unsupported offering aliases', async () => {
    const res = await GET(
      makeRequest('/api/v1/pros/nearby?lat=32.7157&lng=-117.1611&offering=balayage'),
    )
    const body = await res.json()
  ...
  it('passes normalized params to the shared nearby-pros loader', async () => {
    ...
    const req = makeRequest(
      '/api/v1/pros/nearby?lat=32.7157&lng=-117.1611&radiusMiles=25&categoryId=cat_hair&serviceId=svc_1&excludeProfessionalId=pro_1&limit=12',
    )
  ...
  it('returns 500 when the shared loader throws', async () => {
    ...
    const res = await GET(
      makeRequest('/api/v1/pros/nearby?lat=32.7157&lng=-117.1611'),
    )
    const body = await res.json()
