...
function makeGetRequest(id = 'doc_1'): Request {
  return new Request(
    `http://localhost/api/v1/admin/verification-docs/open?id=${encodeURIComponent(
      id,
    )}`,
    {
      method: 'GET',
...
describe('app/api/v1/admin/verification-docs/open/route.ts', () => {
  ...
  it('returns 400 when id is missing', async () => {
    const result = await GET(
      new Request('http://localhost/api/v1/admin/verification-docs/open', {
        method: 'GET',
      }),
    )
  ...
  it('returns generic 500 and logs structured metadata when Supabase signing fails', async () => {
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      'GET /api/v1/admin/verification-docs/open sign error',
      {
  ...
  it('returns 500 and logs structured metadata without leaking unexpected errors to the response', async () => {
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      'GET /api/v1/admin/verification-docs/open error',
      {
        errorName: 'Error',
