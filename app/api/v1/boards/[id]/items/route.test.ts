// app/api/v1/boards/[id]/items/route.test.ts
...
describe('app/api/v1/boards/[id]/items/route.ts', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('adds the canonical lookPostId to the board and returns stable mutation state', async () => {
    const res = await POST(
      new Request('http://localhost/api/v1/boards/board_1/items', {
        method: 'POST',
        body: JSON.stringify({
          lookPostId: 'look_1',
        }),
  ...
  it('returns 400 when the board route param is blank', async () => {
    const res = await POST(
      new Request('http://localhost/api/v1/boards/%20%20/items', {
        method: 'POST',
        body: JSON.stringify({
          lookPostId: 'look_1',
        }),
