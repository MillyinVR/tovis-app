// app/api/v1/boards/[id]/items/[lookId]/route.test.ts
...
describe('app/api/v1/boards/[id]/items/[lookId]/route.ts', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('removes the canonical lookPostId from the board and returns stable mutation state', async () => {
    const res = await DELETE(
      new Request('http://localhost/api/v1/boards/board_1/items/look_1', {
        method: 'DELETE',
      }),
  ...
  it('returns 400 when the board route param is blank', async () => {
    const res = await DELETE(
      new Request('http://localhost/api/v1/boards/%20%20/items/look_1', {
        method: 'DELETE',
      }),
      makeCtx('   ', 'look_1'),
    )
  ...
  it('returns 400 when the look route param is blank', async () => {
    const res = await DELETE(
      new Request('http://localhost/api/v1/boards/board_1/items/%20%20', {
        method: 'DELETE',
      }),
      makeCtx('board_1', '   '),
    )
  ...
  it('returns 404 when the canonical lookPostId does not exist', async () => {
    ...
    const res = await DELETE(
      new Request('http://localhost/api/v1/boards/board_1/items/look_missing', {
        method: 'DELETE',
      }),
  ...
  it('does not treat legacy media ids as fallback identifiers', async () => {
    ...
    const res = await DELETE(
      new Request('http://localhost/api/v1/boards/board_1/items/media_legacy_1', {
        method: 'DELETE',
      }),
