// app/api/client/settings/route.test.ts
...
function makePatchRequest(body: unknown) {
  return new Request('http://localhost/api/client/settings', {
    method: 'PATCH',
    headers: {
...
describe('app/api/client/settings/route', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    vi.useFakeTimers()
  ...
  it('returns sanitized 500 when PATCH throws unexpectedly', async () => {
    ...
    try {
      ...
      expect(consoleError).toHaveBeenCalledWith(
        'PATCH /api/client/settings error',
        expect.any(Error),
      )
    } finally {
      consoleError.mockRestore()
