...
export async function POST(req: Request) {
  try {
    ...
    const { clientId } = auth
    if (!clientId) return jsonFail(401, 'Unauthorized')
    ...
    const path = buildPath({ clientId: String(clientId), kind: 'REVIEW_PUBLIC', contentType })

    const { data, error } = await supabaseAdmin.storage.from(bucket).createSignedUploadUrl(path)

    if (error) return jsonFail(500, error.message || 'Failed to create signed upload URL')
    if (!data?.token) return jsonFail(500, 'Signed upload token missing')
