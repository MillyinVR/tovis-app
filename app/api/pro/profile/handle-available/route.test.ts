// app/api/pro/profile/handle-available/route.test.ts
...
function makeRequest(handle: string) {
  return new Request(
    `http://localhost/api/pro/profile/handle-available?handle=${encodeURIComponent(handle)}`,
  )
}
...
describe('GET /api/pro/profile/handle-available', () => {
  beforeEach(() => {
    vi.clearAllMocks()
