          ...
          <div className="mt-3 inline-flex items-center gap-2 text-xs font-black">
            <span
              className={cn(
                ...
                tone === 'accent' &&
                  'bg-accentPrimary text-bgPrimary border-white/0',
              )}
            >
              {pill(props.count)}
            </span>
            <span className="text-textSecondary group-hover:text-textPrimary transition">
              View →
            </span>
          </div>
        </div>
...
export default async function ClientBookingsHeroCards() {
  ...
  const [upcomingCount, pendingCount, unreadAftercareCount] =
    await Promise.all([
    ...
    <section className="rounded-card border border-white/10 bg-bgSecondary p-4">
      <div className="mb-3 flex items-baseline justify-between gap-3">
        ...
        <div className="text-xs font-semibold text-textSecondary">
          Luxury dashboard. Zero dashboard energy.
        </div>
      </div>
