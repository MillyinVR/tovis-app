...
CREATE FUNCTION public.consult_look_service_start_guard()
...
BEGIN
  ...
  IF estimate IS NULL OR estimate->'firstAppointment'->>'durationMinutes' IS NULL
    ...
    OR EXISTS (SELECT 1 FROM jsonb_array_elements(estimate->'visits'->0->'steps') step
      WHERE NOT EXISTS (SELECT 1 FROM public."BookingServiceItem" item WHERE item."bookingId" = NEW.id
        AND item."offeringId" = step->>'offeringId')) THEN
    RAISE EXCEPTION 'reserved appointment must cover the confirmed look work and time' USING ERRCODE = '23514';
  END IF;
  RETURN NEW;
END $$;
CREATE TRIGGER "Booking_consult_look_service_start" BEFORE UPDATE OF "sessionStep" ON public."Booking"
  FOR EACH ROW EXECUTE FUNCTION public.consult_look_service_start_guard();
